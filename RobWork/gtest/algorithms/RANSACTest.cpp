...
TEST (RANSACTest, RANSACLineTest)
{
    ...
    // read data file
    string filePath = TestEnvironment::testfilesDir () + "ransac/line_data.csv";
    ...
    while (seed < 100) {
        ...
        EXPECT_TRUE (bestModel.same (referenceModel, 0.01));
    }
}

TEST (RANSACTest, RANSACStructuredLineTest)
{
    ...
    // read data file
    string filePath = TestEnvironment::testfilesDir () + "ransac/sline_data.csv";
    ifstream inFile (filePath.c_str ());
    vector< Transform3D<> > data = readData (inFile);
    inFile.close ();
    EXPECT_EQ (data.size () , 15);
    ...
    while (seed < 100) {
        ...
        // find models
        vector< StructuredLineModel > models =
            StructuredLineModel::findModels (pos, 500, 5, 0.2, 0.001);
        ...
        StructuredLineModel referenceModel (
            referenceLine, Vector3D<> (0.68698, -0.43474, 0.60617), 0.944152);

        EXPECT_TRUE (bestModel.same (referenceModel, 0.03));
    }
}

TEST (RANSACTest, RANSACPlaneTest)
{
    ...
    // read data file
    string filePath = TestEnvironment::testfilesDir () + "ransac/plane_data.csv";
    ifstream inFile (filePath.c_str ());
    vector< Transform3D<> > data = readData (inFile);
    inFile.close ();
    EXPECT_EQ (data.size () , 15);
    ...
    while (seed < 100) {
        ...
        // check if the model is acceptably close to ground truth
        rw::geometry::Plane referencePlane (Vector3D<> (-0.25435, -0.962100, 0.098347),
                                            0.25    // distance from plane to origo.
        );
        PlaneModel referenceModel (referencePlane);
        EXPECT_EQ (
            referencePlane.d () ,
            -referenceModel.d ());    // Notice that the PlaneModel uses the distance from the origo
                                      // to the plane (which is opposite of the Plane).

        EXPECT_TRUE (bestModel.same (referenceModel, 0.01));
    }
}

TEST (RANSACTest, RANSACStablePose1DTest)
{
    /* Test whether RANSAC finds a stable pose with 1 dof model in provided data file.
     *
     * Test file contains 10 points with orientations matching a stable pose with noise of
     * sigma=5deg. Test file also contains 5 random outliers.
     */

    // read data file
    string filePath = TestEnvironment::testfilesDir () + "ransac/stablepose_data.csv";
    ifstream inFile (filePath.c_str ());
    vector< Transform3D<> > data = readData (inFile);
    inFile.close ();
    ...
    while (seed < 100) {
        ...
        // check if the model is acceptably close to ground truth
        StablePose1DModel referenceModel (Vector3D<> (0, 0, 1), Vector3D<> (0, 0, 1));

        EXPECT_TRUE (bestModel.same (referenceModel, 0.05));
        ...
        EXPECT_TRUE (modelPlanes.same (modelAxes, 0.01));
    }
}

TEST (RANSACTest, RANSACStablePose0DTest)
{
    ...
    // read data file
    string filePath = TestEnvironment::testfilesDir () + "ransac/stablepose0_data.csv";
    ifstream inFile (filePath.c_str ());
    vector< Transform3D<> > data = readData (inFile);
    inFile.close ();
    EXPECT_GT (data.size () , 0);
    ...
    while (seed < 100) {
        ...
        EXPECT_TRUE (bestModel.same (referenceModel, 1.0));
    }
}
