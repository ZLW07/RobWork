...
void SyncVelocityRampTest(){
    ...
    //Test when reaching the velocity limit
    {
        ...
        EXPECT_LT(fabs(t - 3.5) , 1e-12);
        EXPECT_EQ(ramp.x(0) , q1);
        EXPECT_EQ(ramp.x(t) , q2);
        EXPECT_EQ(ramp.x(t/2.0) , (q1+q2)/2.0);

        EXPECT_EQ(ramp.dx(0) , Q(n,0));
        EXPECT_EQ(ramp.dx(t) , Q(n,0));
        EXPECT_EQ(ramp.dx(t/2) , dqlimit);

        EXPECT_EQ(ramp.ddx(0) , ddqlimit);
        EXPECT_EQ(ramp.ddx(t-0.001) , -ddqlimit);
        EXPECT_EQ(ramp.ddx(t/2.0) , Q(n,0));
...
/*        std::ofstream outfile("velramp.out");
    ...
    //Test when not reaching the velocity limit
    {
        ...
        EXPECT_EQ(ramp.dx(0) , Q(n,0));
        EXPECT_EQ(ramp.dx(t) , Q(n,0));
        EXPECT_EQ(ramp.dx(t/2) , ddqlimit*(-t)/2.0);
