...
TEST(SerialDeciveTest, JointTest)
{
    rw::core::Ptr<RevoluteJoint> rjoint = rw::core::ownedPtr(new RevoluteJoint("RevoluteJointA", Transform3D<>::identity()));

    EXPECT_LT(rjoint->getBounds().first(0) , -1000000.0);
    EXPECT_GT(rjoint->getBounds().second(0) , 1000000.0);

    rw::core::Ptr<PrismaticJoint> pjoint = rw::core::ownedPtr(new PrismaticJoint("PrismaticJointB",Transform3D<>::identity()));
    EXPECT_LT(pjoint->getBounds().first(0) , -1000000.0);
    EXPECT_GT(pjoint->getBounds().second(0) , 1000000.0);
}
...
TEST(SerialDeviceTest, forwardKinematicsTest)
{
    ...
    { // simple forward kinematic of one joint
        ...
        EXPECT_EQ(simple.getQ(state)[0] , Pi/2.0);

        EXPECT_EQ(joint1->getTransform(state).P().e().lpNorm<Eigen::Infinity>() , 0);
        EXPECT_LE((joint1->getTransform(state).R().e() - EAA<>(0.0, 0.0, Pi/2.0).toRotation3D().e()).lpNorm<Eigen::Infinity>() , 1e-6);

        Transform3D<> bTe_s = simple.baseTend(state);

        //std::cout << bTe_s << "\n";

        EXPECT_EQ(bTe_s.P().e().lpNorm<Eigen::Infinity>() , 0);
        EXPECT_LE((bTe_s.R().e() - EAA<>(0.0, 0.0, Pi/2.0).toRotation3D().e()).lpNorm<Eigen::Infinity>() , 1e-6);
    ...
    { // forward kinematics of a serialChain robot
        ...
        EXPECT_LT((bTe1.P() - compare.P()).e().lpNorm<Eigen::Infinity>() , 1e-6);
        EXPECT_LT((bTe1.R().e() - compare.R().e()).lpNorm<Eigen::Infinity>() , 1e-6);

        EXPECT_LT((bTe2.P() - compare.P()).e().lpNorm<Eigen::Infinity>() , 1e-6);
        EXPECT_LT((bTe2.R().e() - compare.R().e()).lpNorm<Eigen::Infinity>() , 1e-6);
...
TEST(SerialDeviceTest, SerialDevice){
    ...
    EXPECT_LT(normInf(eVe - eVe2) , 1e-6 );
    EXPECT_LT(normInf(tVt - tVt2) , 1e-6 );
    ...
    EXPECT_LT(t3dmetric->distance(t_bTe,t_bTf) , 1e-6);
    ...
    EXPECT_LT(normInf(tVt - tVt3) , 1e-6);
