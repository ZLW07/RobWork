...
TEST(PrismaticJointTest, PrismaticJoint) {
	{
		...
		EXPECT_LT((post.P()-pre.P()).normInf() , std::numeric_limits<double>::epsilon());
		EXPECT_LT(EAA<>(post.R()).angle()-EAA<>(pre.R()).angle() , std::numeric_limits<double>::epsilon());
		EXPECT_LT(cross(EAA<>(post.R()).axis(),EAA<>(pre.R()).axis()).normInf() , std::numeric_limits<double>::epsilon());

		// Check values are as expected
		const Rotation3D<> expRot = wTp.R()*rot;
		EXPECT_LT(EAA<>(post.R()).angle()-EAA<>(expRot).angle() , std::numeric_limits<double>::epsilon());
		EXPECT_LT(cross(EAA<>(post.R()).axis(),EAA<>(expRot).axis()).normInf() , std::numeric_limits<double>::epsilon());

		const Vector3D<> expPos = wTp.P()+wTp.R().getCol(2)*0.1+expRot.getCol(2)*q[0];
		EXPECT_LT((post.P()-expPos).normInf() , std::numeric_limits<double>::epsilon());
	}
