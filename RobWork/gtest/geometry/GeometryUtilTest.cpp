...
TEST(GeometryUtilTest, EstimateVolumeTest ){
	...
	}
	{
		...
		EXPECT_NEAR(volEst,Pi*radius*radius*height,1);
	}
	{
		// Pyramid test
		const double x = 0.02;
		...
		EXPECT_NEAR(volEst,height*x*y/3.,1e-5);
	}
	{
		// Cone test
		const double height = 0.1;
		...
		EXPECT_NEAR(volEst,Pi*height*(rBot*rBot+rTop*rTop+rBot*rTop)/3.,1);
	}
	{
		// Tube test
		const double radius = 0.02;
		...
		EXPECT_NEAR(volEst,Pi*height*((radius+thickness)*(radius+thickness)-radius*radius),1);
	}
	{
		// STL table test
		...
		{
			const std::string file = TestEnvironment::testfilesDir() + "geoms/table.stl";
#if RW_HAVE_ASSIMP
			LoaderAssimp loader;
			...
			EXPECT_NEAR(volEst,volTot,3e-5);
		}

		{
			const std::string file = TestEnvironment::testfilesDir() + "geoms/table_union.stl";
#if RW_HAVE_ASSIMP
			LoaderAssimp loader;
			const Model3D::Ptr model = loader.load(file);
			const GeometryData::Ptr gdata = model->toGeometryData();
		...
		// Multiple geometry test (the table again)
		{
			...
			EXPECT_NEAR(volEst,volTot,2e-5);
		}
	}
}

TEST(GeometryUtilTest, EstimateCOGTest ){
	{
		// Sphere test
		const double radius = 0.02;
		const Sphere sphere(radius);
		const Vector3D<> cogEst = GeometryUtil::estimateCOG(*sphere.createMesh(6));
		EXPECT_LT(dot(cogEst,Vector3D<>::x()),std::numeric_limits<double>::epsilon());
		EXPECT_LT(dot(cogEst,Vector3D<>::y()),std::numeric_limits<double>::epsilon());
		EXPECT_LT(dot(cogEst,Vector3D<>::z()),std::numeric_limits<double>::epsilon());
	}
	{
		// Box test
		const double x = 0.02;
	...
	}
	{
		...
		EXPECT_LT(dot(cogEst,Vector3D<>::x()),std::numeric_limits<double>::epsilon());
		EXPECT_LT(dot(cogEst,Vector3D<>::y()),std::numeric_limits<double>::epsilon());
		EXPECT_NEAR(dot(cogEst,Vector3D<>::z()),1./4.*height,1e-5);
	}
	{
		// Cone test
		const double height = 0.1;
	...
	}
	{
...

		{
			...
			EXPECT_LT(dot(cogEst,Vector3D<>::x()),std::numeric_limits<double>::epsilon());
			EXPECT_LT(dot(cogEst,Vector3D<>::y()),std::numeric_limits<double>::epsilon());
			EXPECT_NEAR(dot(cogEst,Vector3D<>::z()),cogZ,1e-5);
		}
...
TEST(GeomtryUtilTest, EstimateInertiaTest ){
	...
	const Transform3D<> ref = Transform3D<>::identity();
	{
		...
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::x(),Vector3D<>::x()),I,0.1);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::y(),Vector3D<>::y()),I,0.1);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::z(),Vector3D<>::z()),I,0.1);
	}
	{
		// Box test
		const double x = 0.02;
		...
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::x(),Vector3D<>::x()),Ix,1e-5);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::y(),Vector3D<>::y()),Iy,1e-5);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::z(),Vector3D<>::z()),Iz,1e-5);
	}
	{
		// Cylinder test
		const double radius = 0.02;
		...
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::x(),Vector3D<>::x()),Ix,0.2);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::y(),Vector3D<>::y()),Ix,0.2);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::z(),Vector3D<>::z()),Iz,1);
	}
	{
		// Pyramid test
		const double x = 0.02;
	...
	}
	{
		...
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::x(),Vector3D<>::x()),Ix,0.6);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::y(),Vector3D<>::y()),Ix,0.6);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::z(),Vector3D<>::z()),Iz,1);
	}
	{
		// Tube test
		...
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::x(),Vector3D<>::x()),Ix,0.25);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::y(),Vector3D<>::y()),Ix,0.25);
		EXPECT_NEAR(dot(inertiaEst*Vector3D<>::z(),Vector3D<>::z()),Iz,1);
	}
	{
		// STL table test
...

		{
			...
			const InertiaMatrix<> inertiaEst = GeometryUtil::estimateInertia(mass, geoms, cog);
			EXPECT_NEAR(dot(inertiaEst*Vector3D<>::x(),Vector3D<>::x()),Ix,2e-5);
			EXPECT_NEAR(dot(inertiaEst*Vector3D<>::y(),Vector3D<>::y()),Ix,2e-5);
			EXPECT_NEAR(dot(inertiaEst*Vector3D<>::z(),Vector3D<>::z()),Iz,2e-5);
		}
...
TEST(GeometryUtilTest, EstimateInertiaCOGTest ){
	const double mass = 0.5;
	const Transform3D<> ref = Transform3D<>::identity();
