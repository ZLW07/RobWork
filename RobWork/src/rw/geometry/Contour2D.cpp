...
double Contour2D::calcArea ()
{
    ...
    for (int p = n - 1, q = 0; q < n; p = q++) {
        area +=
            _points[p].P () (0) * _points[q].P () (1) - _points[q].P () (0) * _points[p].P () (1);
    }
    return area * 0.5f;
}
...
struct MovingAverage
{
  public:
    ...
    Vector2D<> getAverage () const { return _sum / _len; }
...
double Contour2D::calcSequenceMoment (const Contour2D& contour, const rw::math::Vector2D<>& c,
                                      const int r)
{
    double mr = 0;
    for (const Contour2D::Point& p : contour.points ()) {
...
double Contour2D::calcCentralMoments (const Contour2D& contour, const rw::math::Vector2D<>& c,
                                      const int r)
{
...
Covariance<> Contour2D::calcCovarianceMatrix (const Contour2D& contour,
                                              const rw::math::Vector2D<>& c)
{
    Eigen::MatrixXd covarM = Eigen::MatrixXd::Zero (2, 2);
    for (const Contour2D::Point& cp : contour.points ()) {
...
Rotation2D<> Contour2D::calcOrientation (const Contour2D& contour, const rw::math::Vector2D<>& c)
{
...
rw::math::Vector2D<> Contour2D::calcNormal (int idx, int pixelStep, const Contour2D& contour,
                                            bool counterClock)
{
...
void contourFaces (const Contour2D& object, double z, const Vector3D< float >& normal,
                   PlainTriMeshN1F::Ptr mesh)
{
    std::vector< Vector2D<> > pnts;
    for (const Contour2D::Point& pnt : object.points ()) {
        pnts.push_back (Vector2D<> ((float) pnt.P () (0), (float) pnt.P () (1)));
    }
    ...
    for (size_t i = 0; i < triangulation.size (); i += 3) {
        ...
        mesh->add (TriangleN1< float > (Vector3D< float > ((float) p1[0], (float) p1[1], (float) z),
...
void sideFaces (const Contour2D& object, double height, PlainTriMeshN1F::Ptr mesh)
{
