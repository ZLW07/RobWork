...
namespace rw { namespace geometry {
     ...
     *
     */
    template< class T = size_t > class IndexedTriArray : public rw::geometry::TriMesh
    {
      private:
      ...
      public:
        ...
        IndexedTriArray (TriMesh::Ptr objArr) :
            ...
            _centerArr (rw::core::ownedPtr (
                new std::vector< rw::math::Vector3D< float > > (objArr->getSize ()))),
         ...
         */
        IndexedTriArray (
            TriMesh::Ptr objArr, rw::core::Ptr< std::vector< T > > idxArr,
            rw::core::Ptr< std::vector< rw::math::Vector3D< float > > > centerArr,
            rw::core::Ptr< std::vector< float > > valArr,
            rw::core::Ptr< std::vector< boost::tuple< T, float, rw::math::Vector3D< float > > > >
                valCenterArr,
      ...
      private:
        struct TrisIdxSort
        {
            TrisIdxSort (const int splitAxis, const rw::math::Transform3D<>& t3d,
                         const std::vector< rw::math::Vector3D< float > >& centerArr) :
                _splitAxis (splitAxis),
                _t3d (t3d), _centers (centerArr)
            {
            ...
            bool operator() (const size_t& i0, const size_t& i1)
            {
                ...
                */
                // this basiacally takes one column of the rotation matrix and performs only the
                // relevant multiplications in v = T*center
        ...
        struct TrisIdxSort2
        {
            TrisIdxSort2 (const std::vector< float >& valArr) : _valArr (valArr) {}
        ...
        struct IdxValSort
        {
            ...
            bool operator() (const boost::tuple< T, float, rw::math::Vector3D< float > >& i0,
                             const boost::tuple< T, float, rw::math::Vector3D< float > >& i1)
            {
      ...
      public:
        ...
        void sortAxis (int axis, const rw::math::Transform3D<>& t3d)
        {
            ...
            for (size_t i = _first; i < _first + size (); i++) {
                ...
                boost::tuples::get< 1 > (elem) =
                    ic0[0] * R_s0 + ic0[1] * R_s1 + ic0[2] * R_s2 + P_s;    //(t3df*ic0)(axis);
            }
            ...
            // std::sort( &((*_idxArr)[_first]), &((*_idxArr)[_first])+ size(), TrisIdxSort2(axis,
            // t3d, *_centerArr));
            // std::sort( &((*_idxArr)[_first]), &((*_idxArr)[_first])+ size(),
            // TrisIdxSort2(*_valArr));
            std::sort (
                &((*_valCenterArr)[_first]), &((*_valCenterArr)[_first]) + size (), IdxValSort ());
        ...
        IndexedTriArray< T > getSubRange (size_t first, size_t last)
        {
            ...
            return IndexedTriArray< T > (_objArr,
                                         _idxArr,
                                         _centerArr,
                                         _valArr,
                                         _valCenterArr,
        ...
        {
            return rw::core::ownedPtr (new IndexedTriArray< T > (
                _objArr, _idxArr, _centerArr, _valArr, _valCenterArr, _first, _last));
        }
        ...
        inline size_t getGlobalIndex (int idx) const
        {
        ...
        rw::geometry::GeometryData::GeometryType getType () const { return GeometryData::UserType; }

        //! @copydoc TriMesh::getSize
        size_t getSize () const { return _last - _first; }
