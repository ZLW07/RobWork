...
namespace rw { namespace geometry {
    ...
    template< class T = double > class IndexedTriMesh : public rw::geometry::TriMesh
    {
      ...
      protected:
        ...
        IndexedTriMesh (rw::core::Ptr< VertexArray > vertices, rw::core::Ptr< VertexArray > normals,
            ...
            _vertPtr (vertices), _normals (normals.get ()), _normPtr (normals), _triIdxArr (NULL),
            _stride (triStride), _idxsize (idxsize)
        {}
      ...
      private:
        IndexedTriMesh (IndexedTriMesh< T >& mesh) :
            _vertices (NULL), _normals (NULL), _triIdxArr (NULL), _stride (0), _idxsize (0)
        {}    // this is illegal
    ...
    class IndexedTriMeshN0 : public rw::geometry::IndexedTriMesh< T >
    {
      ...
      public:
        //! @brief Constructor
        IndexedTriMeshN0 () :
            IndexedTriMesh< T > (rw::core::ownedPtr (new VertexArray ()),
        ...
        IndexedTriMeshN0 (VertexArray* vertices) :
            IndexedTriMesh< T > (vertices, rw::core::ownedPtr (new VertexArray ()),
                                 (uint8_t) sizeof (tri_type), (uint8_t) sizeof (S)),
            _triangles (rw::core::ownedPtr (new TriangleArray ()))
        {}
        ...
        IndexedTriMeshN0 (rw::core::Ptr< VertexArray > vertices,
            ...
            IndexedTriMesh< T > (vertices, normals, (uint8_t) sizeof (tri_type),
                                 (uint8_t) sizeof (S)),
            _triangles (rw::core::ownedPtr (new TriangleArray ()))
        {}
         ...
         * @brief constructor
         * @param vertices [in] pointer to vector of vertices.
         * @param normals [in] pointer to vector of normals (expects same length as either \b
         * vertices or \b triangles).
        ...
        const rw::math::Vector3D< T >& getVertexNormal (size_t i, VertexIdx vidx) const
        {
            const uint32_t idx = static_cast< uint32_t > (
                this->_stride * i + vidx * this->_idxsize);    // this is the unmasked idx
        ...
        void getTriangle (size_t i, rw::geometry::Triangle< double >& dst) const
        {
            ...
            if (std::is_same< float, T >::value) {
                ...
                dst[0] =
                    *(reinterpret_cast< const Vector3D< double >* > (&(*this->_vertices)[v0idx]));
                dst[1] =
                    *(reinterpret_cast< const Vector3D< double >* > (&(*this->_vertices)[v1idx]));
                dst[2] =
                    *(reinterpret_cast< const Vector3D< double >* > (&(*this->_vertices)[v2idx]));
            }
        }
        ...
        void getTriangle (size_t i, rw::geometry::Triangle< float >& dst) const
        {
            ...
            if (std::is_same< float, T >::value) {
                ...
                dst[1] =
                    *(reinterpret_cast< const Vector3D< float >* > (&(*this->_vertices)[v1idx]));
                dst[2] =
                    *(reinterpret_cast< const Vector3D< float >* > (&(*this->_vertices)[v2idx]));
            }
            else {
         ...
         */
        TriangleArray& getTriangles () { return *_triangles; }
         ...
         */
        tri_type& operator[] (int i) { return (*_triangles)[i]; }
