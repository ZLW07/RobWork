...
ImplicitTorus::ImplicitTorus (double R, double r) :
    _R1 (R), _R2 (R), _r1 (r), _r2 (r), _stepsPerRevolution (10), _isNormalForm (true)
{}

ImplicitTorus::ImplicitTorus (double R1, double R2, double r1, double r2) :
    _R1 (R1), _R2 (R2), _r1 (r1), _r2 (r2), _stepsPerRevolution (10), _isNormalForm (true)
{}
...
ImplicitTorus::ImplicitTorus (double R1, double R2, double r1, double r2,
    ...
    _R1 (R1),
    _R2 (R2), _r1 (r1), _r2 (r2), _conditions (conditions),
    _stepsPerRevolution (stepsPerRevolution), _transform (transform), _isNormalForm (false)
{}
...
ImplicitTorus::Ptr ImplicitTorus::transform (const Transform3D<>& T) const
{
    ...
    return ownedPtr (
        new ImplicitTorus (_R1, _R2, _r1, _r2, _transform * T, conditions, _stepsPerRevolution));
}
...
ImplicitTorus::Ptr ImplicitTorus::transform (const Vector3D<>& P) const
{
    ...
    for (std::size_t i = 0; i < _conditions.size (); i++)
        conditions.push_back (_conditions[i]->transform (T));
    return ownedPtr (new ImplicitTorus (_R1, _R2, _r1, _r2, T, conditions, _stepsPerRevolution));
}
...
ImplicitTorus::Ptr ImplicitTorus::scale (double factor) const
{
    ...
    return ownedPtr (new ImplicitTorus (_R1 * factor,
                                        _R2 * factor,
                                        _r1 * factor,
                                        _r2 * factor,
                                        _transform,
...
ImplicitTorus::Ptr ImplicitTorus::clone () const
{
    ...
    return ownedPtr (
        new ImplicitTorus (_R1, _R2, _r1, _r2, _transform, conditions, _stepsPerRevolution));
}
...
TriMesh::Ptr ImplicitTorus::getTriMesh (const std::vector< Vector3D<> >& border) const
{
    ...
    const ImplicitTorus normalForm (
        _R1, _R2, _r1, _r2, _transform, conditions, _stepsPerRevolution);
    return normalForm.getTriMeshNormalForm (borderRot, _transform);
}

TriMesh::Ptr ImplicitTorus::getTriMeshNormalForm (const std::vector< Vector3D<> >& border,
                                                  const Transform3D<>& transform) const
{
    ...
    // If there are front border segments lying completely inside the +/- EPS region, remove them if
    // they are part of a larger back segment.
    {
        ...
        for (std::list< std::vector< std::size_t > >::iterator it = listA.begin ();
            ...
            if (within) {
                bool match = false;
                for (std::list< std::vector< std::size_t > >::const_iterator itB = listB.begin ();
                     itB != listB.end () && !match;
                     itB++) {
                    bool outsideB = false;
    ...
    // If there are back border segments lying completely inside the +/- EPS region, remove them if
    // they are part of a larger front segment.
    {
    ...
    silhouette.push_back (QuadraticCurve (Vector3D<>::zero (),
                                          Vector3D<>::x () * (_R1 - _r1),
                                          -Vector3D<>::y () * (_R2 - _r1),
                                          QuadraticCurve::Elliptic));
    silhouette.push_back (QuadraticCurve (Vector3D<>::zero (),
                                          Vector3D<>::x () * (_R1 + _r1),
                                          Vector3D<>::y () * (_R2 + _r1),
                                          QuadraticCurve::Elliptic));
...
void ImplicitTorus::makeSurface (const std::vector< Vector3D<> > fullPolygon, Place place,
                                 PlainTriMeshN1D::Ptr mesh) const
{
    // Project border
...
double ImplicitTorus::operator() (const Vector3D<>& x) const
{
    ...
    if (_R1 == _R2) {
        ...
        const double l    = (R2sq + rTerm) * x0sq + (R1sq + rTerm) * x1sq -
                         (R1sq + rTerm) * (R2sq + rTerm) - 4. * R1R2 * rTerm;
...
Vector3D<> ImplicitTorus::gradient (const Vector3D<>& x) const
{
    ...
    if (_R1 == _R2) {
        ...
        g[2] = 2. * l * drTerm * (x0sq + x1sq - (R1sq + R2sq) - 2. * rTerm - 4. * R1R2) -
               4. * r * r * drTerm + 8. * r * (_R1 + _R2) * rTerm * drTerm;
        return g;
    }
}
