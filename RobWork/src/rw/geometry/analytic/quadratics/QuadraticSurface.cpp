...
QuadraticSurface::QuadraticSurface (const Eigen::Diagonal< Eigen::Matrix3d >& A,
                                    const Eigen::Vector3d& a, double u,
                                    const std::vector< TrimmingRegion >& conditions) :
    _A (Eigen::DiagonalMatrix< double, 3, 3 > (A)),
    _a (a), _u (u), _determinantA (A.determinant ()), _diagonal (true), _conditions (conditions),
    _stepsPerRevolution (10)
{}

QuadraticSurface::QuadraticSurface (const Eigen::DiagonalMatrix< double, 3, 3 >& A,
                                    const Eigen::Vector3d& a, double u,
                                    const std::vector< TrimmingRegion >& conditions) :
    _A (A),
    _a (a), _u (u), _determinantA (A.diagonal ().determinant ()), _diagonal (true),
    _conditions (conditions), _stepsPerRevolution (10)
{}

QuadraticSurface::QuadraticSurface (
    const Eigen::SelfAdjointView< const Eigen::Matrix3d, Eigen::Upper >& A,
    const Eigen::Vector3d& a, double u, const std::vector< TrimmingRegion >& conditions) :
    _A (A),
    _a (a), _u (u), _determinantA (_A.determinant ()), _diagonal (false), _conditions (conditions),
    _stepsPerRevolution (10)
{}

QuadraticSurface::QuadraticSurface (
    const Eigen::SelfAdjointView< Eigen::Matrix3d, Eigen::Upper >& A, const Eigen::Vector3d& a,
    double u, const std::vector< TrimmingRegion >& conditions) :
    _A (A),
    _a (a), _u (u), _determinantA (_A.determinant ()), _diagonal (false), _conditions (conditions),
    _stepsPerRevolution (10)
{}
...
QuadraticSurface::QuadraticSurface (
    const Eigen::SelfAdjointView< Eigen::Matrix3d, Eigen::Lower >& A, const Eigen::Vector3d& a,
    double u, const std::vector< TrimmingRegion >& conditions) :
    _A (A),
    _a (a), _u (u), _determinantA (_A.determinant ()), _diagonal (false), _conditions (conditions),
    _stepsPerRevolution (10)
...
QuadraticSurface::QuadraticSurface (const Eigen::Matrix3d& A, bool diagonal, double determinantA,
                                    const Eigen::Vector3d& a, double u,
                                    const std::vector< TrimmingRegion >& conditions,
                                    double stepsPerRevolution) :
    _A (A),
    _a (a), _u (u), _determinantA (determinantA), _diagonal (diagonal), _conditions (conditions),
...
QuadraticSurface::Ptr QuadraticSurface::transform (const Transform3D<>& T) const
{
    ...
    return ownedPtr (new QuadraticSurface (Anew,
                                           false,
                                           _determinantA,
                                           R * _a - Anew * p,
                                           (Anew * p - 2 * (R * _a)).transpose () * p + _u,
                                           conditions,
                                           _stepsPerRevolution));
}
...
QuadraticSurface::Ptr QuadraticSurface::transform (const Vector3D<>& P) const
{
    ...
    return ownedPtr (new QuadraticSurface (_A,
                                           _diagonal,
                                           _determinantA,
                                           _a - _A * p,
                                           (_A * p - 2 * _a).transpose () * p + _u,
                                           conditions,
                                           _stepsPerRevolution));
}
...
QuadraticSurface::Ptr QuadraticSurface::clone () const
{
    ...
    return ownedPtr (new QuadraticSurface (
        _A, _diagonal, _determinantA, _a, _u, conditions, _stepsPerRevolution));
}
...
QuadraticSurface::Ptr QuadraticSurface::normalize () const
{
    ...
    const double normFactor =
        std::max (_A.cwiseAbs ().maxCoeff (), std::max (_a.cwiseAbs ().maxCoeff (), std::abs (_u)));
    return ownedPtr (new QuadraticSurface (_A / normFactor,
                                           _diagonal,
                                           _determinantA / std::pow (normFactor, 3),
                                           _a / normFactor,
                                           _u / normFactor,
                                           conditions,
                                           _stepsPerRevolution));
...
std::pair< QuadraticSurface, Rotation3D<> > QuadraticSurface::diagonalize () const
{
    ...
    return std::make_pair (
        QuadraticSurface (Eigen::DiagonalMatrix< double, 3, 3 > (eigen.eigenvalues ()),
                          _diagonal,
                          _determinantA,
                          _a.transpose () * inverse (R).e (),
                          _u,
                          conditions,
                          _stepsPerRevolution),
        R);
...
TriMesh::Ptr QuadraticSurface::getTriMeshDiagonal (const std::vector< Vector3D<> >& border,
                                                   const Rotation3D<>& R) const
{
    ...
    if (std::fabs (_A (e, e)) >= EPS) {
        ...
        // Front
        {
            ...
            if (last[e] >= eSplit - EPS) {
                if (border.front ()[e] >= eSplit - EPS && borderFront.size () > 0) {
                    borderFront.front ().insert (
                        borderFront.front ().begin (), cur.begin (), cur.end ());
                    cur.clear ();
                }
            }
            if (cur.size () > 0) {
        ...
        // Back
        {
            ...
            if (last[e] <= eSplit + EPS) {
                if (border.front ()[e] <= eSplit + EPS && borderBack.size () > 0) {
                    borderBack.front ().insert (
                        borderBack.front ().begin (), cur.begin (), cur.end ());
                    cur.clear ();
                }
            }
            if (cur.size () > 0) {
        ...
        // If there are front border segments lying completely inside the +/- EPS region, remove
        // them if they are part of a larger back segment.
        {
            std::list< std::vector< std::size_t > >& listA       = borderFront;
            const std::list< std::vector< std::size_t > >& listB = borderBack;

            for (std::list< std::vector< std::size_t > >::iterator it = listA.begin ();
                 it != listA.end ();) {    // do not use const_iterator (first supported with
                                           // list.erase() from GCC 4.9)
                bool within = true;
                ...
                if (within) {
                    bool match = false;
                    for (std::list< std::vector< std::size_t > >::const_iterator itB =
                             listB.begin ();
                         itB != listB.end () && !match;
                         itB++) {
                        bool outsideB = false;
        ...
        // If there are back border segments lying completely inside the +/- EPS region, remove them
        // if they are part of a larger front segment.
        {
        ...
        const std::list< std::vector< Vector3D<> > > fullPolygonFront =
            AnalyticUtil::combinePolygons (border, borderFront, silhouette, _stepsPerRevolution);
        const std::list< std::vector< Vector3D<> > > fullPolygonBack =
            AnalyticUtil::combinePolygons (border, borderBack, silhouette, _stepsPerRevolution);

        for (std::list< std::vector< Vector3D<> > >::const_iterator it = fullPolygonFront.begin ();
             it != fullPolygonFront.end ();
             it++) {
            makeSurface (*it, u, v, e, eSplit, FRONT, R, mesh);
        }
        for (std::list< std::vector< Vector3D<> > >::const_iterator it = fullPolygonBack.begin ();
             it != fullPolygonBack.end ();
             it++) {
...
std::vector< QuadraticCurve > QuadraticSurface::findSilhouette (std::size_t u, std::size_t v,
                                                                std::size_t e, double eSplit) const
{
    // Find silhouette of surface
    ...
    if (std::fabs (_A (u, u)) < EPS && std::fabs (_a[u]) < EPS) {
        ...
        if (s >= 0) {
            ...
            const Vector3D<> c1 ((u == 0) ? 0 : (v == 0) ? vVal1 : eSplit,
                                 (u == 1) ? 0 : (v == 1) ? vVal1 : eSplit,
                                 (u == 2) ? 0 : (v == 2) ? vVal1 : eSplit);
            const Vector3D<> c2 ((u == 0) ? 0 : (v == 0) ? vVal2 : eSplit,
                                 (u == 1) ? 0 : (v == 1) ? vVal2 : eSplit,
                                 (u == 2) ? 0 : (v == 2) ? vVal2 : eSplit);
        ...
        if (s >= 0) {
            ...
            const Vector3D<> c1 ((u == 0) ? uVal1 : (v == 0) ? 0 : eSplit,
                                 (u == 1) ? uVal1 : (v == 1) ? 0 : eSplit,
                                 (u == 2) ? uVal1 : (v == 2) ? 0 : eSplit);
            const Vector3D<> c2 ((u == 0) ? uVal2 : (v == 0) ? 0 : eSplit,
                                 (u == 1) ? uVal2 : (v == 1) ? 0 : eSplit,
                                 (u == 2) ? uVal2 : (v == 2) ? 0 : eSplit);
            const Vector3D<> uDir ((u == 0) ? 1 : 0, (u == 1) ? 1 : 0, (u == 2) ? 1 : 0);
            const Vector3D<> vDir ((v == 0) ? 1 : 0, (v == 1) ? 1 : 0, (v == 2) ? 1 : 0);
        ...
        const double b       = std::sqrt (
            (_a[ru] * _a[ru] / _A (ru, ru) * 2 + _a[rv] * _a[rv] / _A (rv, rv) - Ke) / _A (rv, rv));
        const double a = std::sqrt (b * b * _A (rv, rv) / _A (ru, ru));
        Vector3D<> c;
        Vector3D<> uDir;
        Vector3D<> vDir;
        if (_A (u, u) > 0) {
            c    = Vector3D<> ((u == 0) ? ruValC : (v == 0) ? rvValC : eSplit,
                            (u == 1) ? ruValC : (v == 1) ? rvValC : eSplit,
                            (u == 2) ? ruValC : (v == 2) ? rvValC : eSplit);
            uDir = Vector3D<> ((v == 0) ? 1 : 0, (v == 1) ? 1 : 0, (v == 2) ? 1 : 0) * a;
            vDir = Vector3D<> ((u == 0) ? 1 : 0, (u == 1) ? 1 : 0, (u == 2) ? 1 : 0) * b;
        }
        else {
            c    = Vector3D<> ((u == 0) ? rvValC : (v == 0) ? ruValC : eSplit,
                            (u == 1) ? rvValC : (v == 1) ? ruValC : eSplit,
                            (u == 2) ? rvValC : (v == 2) ? ruValC : eSplit);
...
void QuadraticSurface::makeSurface (const std::vector< Vector3D<> > fullPolygon, std::size_t u,
                                    std::size_t v, std::size_t e, double eSplit, Place place,
                                    const Rotation3D<>& R, PlainTriMeshN1D::Ptr mesh) const
{
    // Project border
    ...
    const std::vector< std::vector< std::size_t > > convexPolygonsIndexed =
        PolygonUtil::convexDecompositionIndexed (polygon);

    std::vector< Polygon< Vector2D<> > > convexPolygons (convexPolygonsIndexed.size ());
    {
        // Construct plain polygon
        std::size_t i = 0;
        for (std::vector< std::vector< std::size_t > >::const_iterator it =
                 convexPolygonsIndexed.begin ();
             it != convexPolygonsIndexed.end ();
             it++) {
            ...
            for (std::size_t k = 0; k < subPoly.size (); k++) {
                ...
                bool inOther              = false;
                for (std::vector< std::vector< std::size_t > >::const_iterator itB =
                         convexPolygonsIndexed.begin ();
                     itB != convexPolygonsIndexed.end () && inOther == false;
                     itB++) {
                    if (itB == it)
                        continue;
    ...
    for (std::size_t i = 0; i < convexPolygons.size (); i++) {
        ...
        if (std::fabs (_A (2, 2)) < EPS && std::fabs (_A (1, 1)) < EPS &&
            std::fabs (_A (0, 0)) < EPS) {
            // Unproject
            for (std::size_t triI = 0; triI < meshProj->size (); triI++) {
                ...
                for (std::size_t pi = 0; pi < 3; pi++) {
                    ...
                    tri[pi] = R * Vector3D<> ((u == 0) ? uVal : (v == 0) ? vVal : eVal,
                                              (u == 1) ? uVal : (v == 1) ? vVal : eVal,
                                              (u == 2) ? uVal : (v == 2) ? vVal : eVal);
                }
                if (br)
                    continue;
                ...
                const Vector3D<> normal = R * Vector3D<> ((u == 0) ? nu : (v == 0) ? nv : ne,
                                                          (u == 1) ? nu : (v == 1) ? nv : ne,
                                                          (u == 2) ? nu : (v == 2) ? nv : ne);
            ...
            for (std::size_t triI = 0; triI < meshProj->size (); triI++) {
                ...
                for (std::size_t pi = 0; pi < 3; pi++) {
                    const double uVal = tri[pi][0];
                    const double vVal = tri[pi][1];
                    double eVal       = -_A (u, u) * uVal * uVal - _A (v, v) * vVal * vVal -
                                  _a[u] * 2 * uVal - _a[v] * 2 * vVal - _u;
                    eVal /= _a[e] * 2;
                    if ((place == FRONT && eVal < eSplit) || (place == BACK && eVal > eSplit)) {
                        br = true;
                        break;
            ...
            for (std::size_t triI = 0; triI < meshProj->size (); triI++) {
                ...
                for (std::size_t pi = 0; pi < 3; pi++) {
                    const double uVal = tri[pi][0];
                    const double vVal = tri[pi][1];
                    double eVal       = _a[e] * _a[e] / _A (e, e) / _A (e, e) -
                                  (_A (u, u) * uVal * uVal + _A (v, v) * vVal * vVal +
                                   _a[u] * uVal * 2 + _a[v] * vVal * 2 + _u) /
                                      _A (e, e);
                    if (eVal >= 0) {
                        if (place == FRONT)
                        ...
                        // RW_WARN("For triangle " << tri << ", the explicit value under square root
                        // was negative (" << eVal << ") assuming split value!");
                        zero = true;
                        // br = true;
                        eVal = eSplit;
                    }
                    if ((place == FRONT && eVal < eSplit) || (place == BACK && eVal > eSplit)) {
                        // RW_WARN("For triangle " << tri << ", the explicit value was not on right
                        // side front/back (" << eVal << ")!");
                        eVal = eSplit;
                        br   = true;
                        break;
                    }
                ...
                Vector3D<> normal;
                if (!zero)
                    normal = R * Vector3D<> ((u == 0) ? nu : (v == 0) ? nv : ne,
                                             (u == 1) ? nu : (v == 1) ? nv : ne,
                                             (u == 2) ? nu : (v == 2) ? nv : ne);
                else if (place == FRONT)
                    ...
                    normal =
                        R * Vector3D<> ((e == 0) ? -1 : 0, (e == 1) ? -1 : 0, (e == 2) ? -1 : 0);
                if (dot (tri.calcFaceNormal (), normal) < 0) {
                    mesh->add (TriangleN1<> (tri[0], tri[2], tri[1], normal));
                }
                else {
    ...
    if (std::fabs (_A (e, e)) < EPS && std::fabs (_a[e]) > EPS) {
        ...
        if (std::fabs (_A (u, u)) < EPS || std::fabs (_A (v, v)) < EPS) {
            ...
            if (_A (u, u) * _A (v, v) < 0) {
                ...
                const double xe = -(_A (u, u) * xu * xu + _A (v, v) * xv * xv + _a[u] * xu * 2 +
                                    _a[v] * xv * 2 + _u) /
                                  (_a[e] * 2);
                const Vector3D<> P = Vector3D<> ((u == 0) ? xu : (v == 0) ? xv : xe,
                                                 (u == 1) ? xu : (v == 1) ? xv : xe,
                                                 (u == 2) ? xu : (v == 2) ? xv : xe);
        ...
        if (((_A (u, u) >= EPS) || (_A (u, u) < EPS && _a[u] < EPS)) &&
            ((_A (v, v) >= EPS) || (_A (v, v) < EPS && _a[v] < EPS))) {
            if (std::fabs (dir[u]) < EPS && std::fabs (dir[v]) < EPS) {
                const double xu = (_A (u, u) < EPS) ? 0 : -_a[u] / _A (u, u);
                const double xv = (_A (v, v) < EPS) ? 0 : -_a[v] / _A (v, v);
                double s        = _a[e] / _A (e, e) * _a[e] / _A (e, e) -
                           (_A (u, u) * xu * xu + _A (v, v) * xv * xv + _a[u] * xu * 2 +
                            _a[v] * xv * 2 + _u) /
                               _A (e, e);
                if (s < 0)
                ...
                const double K1 =
                    _a[e] * _a[e] * Auv +
                    _A (e, e) * (_A (u, u) * _a[v] * _a[v] + _a[u] * _a[u] * _A (v, v) - Auv * _u);
                const double K2 = Auv * dir[e] * dir[e] + _A (e, e) * _A (v, v) * dir[u] * dir[u] +
                                  _A (e, e) * _A (u, u) * dir[v] * dir[v];
                const double su =
                    (_A (u, u) < EPS) ? 0 : dir[u] * dir[u] * K1 / (_A (u, u) * _A (u, u) * K2);
                const double sv =
                    (_A (v, v) < EPS) ? 0 : dir[v] * dir[v] * K1 / (_A (v, v) * _A (v, v) * K2);
                if (su >= 0 && sv >= 0) {
                    ...
                    const double s1  = _a[e] / _A (e, e) * _a[e] / _A (e, e) -
                                      (_A (u, u) * xu1 * xu1 + _A (v, v) * xv1 * xv1 +
                                       _a[u] * xu1 * 2 + _a[v] * xv1 * 2 + _u) /
                                          _A (e, e);
                    const double s2 = _a[e] / _A (e, e) * _a[e] / _A (e, e) -
                                      (_A (u, u) * xu2 * xu2 + _A (v, v) * xv2 * xv2 +
                                       _a[u] * xu2 * 2 + _a[v] * xv2 * 2 + _u) /
                                          _A (e, e);
                    if (s1 > 0) {
...
QuadraticSurface::Ptr QuadraticSurface::makeEllipsoid (const double a, const double b,
                                                       const double c)
{
...
QuadraticSurface::Ptr
QuadraticSurface::makeEllipticHyperboloidOneSheet (const double a, const double b, const double c)
{
    const Eigen::DiagonalMatrix< double, 3 > A (b * b * c * c, a * a * c * c, -a * a * b * b);
    static const Eigen::Vector3d avec = Eigen::Vector3d::Zero ();
    const double u                    = -a * a * b * b * c * c;
...
QuadraticSurface::Ptr QuadraticSurface::makeCircularHyperboloidOneSheet (const double a,
                                                                         const double b)
{
...
QuadraticSurface::Ptr
QuadraticSurface::makeEllipticHyperboloidTwoSheets (const double a, const double b, const double c)
{
    const Eigen::DiagonalMatrix< double, 3 > A (b * b * c * c, a * a * c * c, -a * a * b * b);
    static const Eigen::Vector3d avec = Eigen::Vector3d::Zero ();
...
QuadraticSurface::Ptr QuadraticSurface::makeCircularHyperboloidTwoSheets (const double a,
                                                                          const double b)
{
    const Eigen::DiagonalMatrix< double, 3 > A (b * b, a * a, -a * a);
    static const Eigen::Vector3d avec = Eigen::Vector3d::Zero ();
...
QuadraticSurface::Ptr QuadraticSurface::makeEllipticCone (const double a, const double b,
                                                          const double c)
{
    const Eigen::DiagonalMatrix< double, 3 > A (b * b * c * c, a * a * c * c, -a * a * b * b);
    static const Eigen::Vector3d avec = Eigen::Vector3d::Zero ();
