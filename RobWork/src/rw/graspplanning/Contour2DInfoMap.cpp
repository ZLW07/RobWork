...
{
  public:
    MovingAverage (int len) : _len (len), _cb (_len, 0.0), _sum (0.0), _idx (0) {}
    ...
    double getAverage () { return _sum / _len; }
...
Contour2DInfoMap::Contour2DInfoMap (int res) :
    _res (res), _resStep (2 * Pi / res), _resStepInv (1.0 / _resStep)
{}
                                                                ...
                                                                double thres)
{
    ...
    if (startIdx > stopIdx) {
        for (; startIdx < _res; startIdx++) {
            for (Contact2D* cPtr : _normalToContactsMap[startIdx]) {
