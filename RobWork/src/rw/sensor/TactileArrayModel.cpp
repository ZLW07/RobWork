    ...
    _sdata (1, rw::core::ownedPtr (new TactileModelCache ()).cast< rw::kinematics::StateCache > ())
{
    ...
    for (int j = 0; j < h; j++) {
        for (int i = 0; i < w; i++) {
            Vector3D<> p = _vertexGrid[i][j] + _vertexGrid[i + 1][j] + _vertexGrid[i][j + 1] +
                           _vertexGrid[i + 1][j + 1];
...
const rw::math::Transform3D<>& TactileArrayModel::getTransform () const
{
    return _fThmap;
}
...
const TactileArrayModel::ValueMatrix&
TactileArrayModel::getTexelData (const rw::kinematics::State& state) const
{
    return *_sdata.getStateCache< TactileModelCache > (state)->_data;
}

void TactileArrayModel::setTexelData (const TactileArrayModel::ValueMatrix& data,
