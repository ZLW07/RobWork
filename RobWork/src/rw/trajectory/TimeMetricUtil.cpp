...
double TimeMetricUtil::timeDistance (const Q& from, const Q& to, const Q& velocity)
{
...
double TimeMetricUtil::timeDistance (const Q& from, const Q& to, const Device& device)
{
    return timeDistance (from, to, device.getVelocityLimits ());
}

double TimeMetricUtil::timeDistance (const State& from, const State& to, const Device& device)
{
    return timeDistance (device.getQ (from), device.getQ (to), device.getVelocityLimits ());
}

double TimeMetricUtil::timeDistance (const State& from, const State& to, const WorkCell& workcell)
{
