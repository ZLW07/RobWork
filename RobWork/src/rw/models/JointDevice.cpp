                                                   ...
                                                   const State& state) const
{
    JacobianCalculator* jc =
        new JointDeviceJacobianCalculator (const_cast< JointDevice* > (this), _base, frames, state);
    return ownedPtr (jc);
}
...
void JointDevice::setBounds (const std::pair< Q, Q >& bounds)
{
    ...
    for (std::vector< Joint* >::iterator p = _joints.begin (); p != _joints.end (); ++p) {
        ...
        int dof = (*p)->getDOF ();
        if (dof > 0) {
            (*p)->setBounds (std::make_pair (bounds.first.getSubPart (i, dof),
                                             bounds.second.getSubPart (i, dof)));
            i += dof;
        }
    }
}
...
{
    if (q.size () != getDOF ())
        RW_THROW ("setQ() called for device of size " << (int) getDOF () << " and q of size "
                                                      << (int) q.size ());
