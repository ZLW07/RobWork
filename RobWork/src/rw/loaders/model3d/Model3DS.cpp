...
void Model3DS::Load (const std::string& name)
{
    ...
    if (!bin3ds)
        RW_THROW ("Can't open file " << StringUtil::quote (name));
    ...
    for (int j = 0; j < numMaterials; j++) {
        if (Materials.at (j).textured == false) {
            ...
            Materials.at (j).tex =
                TextureData ("RGB", (float) (r / 256.0), (float) (g / 256.0), (float) (b / 256.0));
...
void Model3DS::EditChunkProcessor (long length, long findex)
{
    ...
    while (ftell (bin3ds) < (findex + length - 6)) {
        ...
        switch (h.id) {
            case OBJECT: numObjects++; break;
            case MATERIAL: numMaterials++; break;
            default: break;
        }
    ...
    while (ftell (bin3ds) < (findex + length - 6)) {
        ...
        switch (h.id) {
            ...
            case MAT_AMBIENT:
                ...
                break;
            case MAT_DIFFUSE: DiffuseColorChunkProcessor (h.len, ftell (bin3ds), matindex); break;
            case MAT_SPECULAR:
                // ColorChunkProcessor(h.len, ftell(bin3ds));
            case MAT_TEXMAP:
...
void Model3DS::FacesMaterialsListChunkProcessor (long length, long findex, int objindex,
                                                 int subfacesindex)
{
    ...
    Objects.at (objindex).MatFaces.at (subfacesindex).subFaces.resize (numEntries * 3);
