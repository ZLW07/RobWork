...
DOMPathLoader::DOMPathLoader (const std::string& filename, rw::models::WorkCell::Ptr workcell,
...
DOMPathLoader::DOMPathLoader (std::istream& instream, rw::models::WorkCell::Ptr workcell,
                              const std::string& schemaFileName)
{
    _workcell             = workcell;
    DOMParser::Ptr parser = DOMParser::make ();
...
{
  public:
    ElementReader (WorkCell::Ptr workcell = NULL) { _workcell = workcell; }
...
QPath::Ptr DOMPathLoader::getQPath ()
{
    if (_type != QType)
        RW_THROW (
            "The loaded Path is not of type QPath. Use DOMPathLoader::getType() to read its type");
    return _qPath;
}
...
Vector3DPath::Ptr DOMPathLoader::getVector3DPath ()
{
    if (_type != Vector3DType)
        RW_THROW ("The loaded Path is not of type Vector3DPath. Use DOMPathLoader::getType() to "
                  "read its type");
    return _v3dPath;
}
...
Rotation3DPath::Ptr DOMPathLoader::getRotation3DPath ()
{
    if (_type != Rotation3DType)
        RW_THROW ("The loaded Path is not of type Rotation3DPath. Use DOMPathLoader::getType() to "
                  "read its type");
    return _r3dPath;
}
...
Transform3DPath::Ptr DOMPathLoader::getTransform3DPath ()
{
    if (_type != Transform3DType)
        RW_THROW ("The loaded Path is not of type Transform3DPath. Use DOMPathLoader::getType() to "
                  "read its type");
    return _t3dPath;
}
...
{
    if (_type != StateType)
        RW_THROW ("The loaded Path is not of type StatePath. Use DOMPathLoader::getType() to read "
                  "its type");
    return _statePath;
}
...
{
    if (_type != TimedQType)
        RW_THROW ("The loaded Path is not of type TimedQPath. Use DOMPathLoader::getType() to read "
                  "its type");
    return _timedQPath;
}
...
rw::trajectory::TimedStatePath::Ptr DOMPathLoader::getTimedStatePath ()
{
    if (_type != TimedStateType)
        RW_THROW ("The loaded Path is not of type TimedStatePath. Use DOMPathLoader::getType() to "
                  "read its type");
