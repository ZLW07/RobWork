...
ProximitySetup DOMProximitySetupLoader::load (const std::string& filename,
...
ProximitySetup DOMProximitySetupLoader::load (std::istream& instream,
                                              const std::string& schemaFileName)
{
    DOMParser::Ptr parser = DOMParser::make ();
    parser->setSchema (schemaFileName);
        ...
        if (child->isName ("Exclude")) {
            ...
            if (!child->isName ("<xmlcomment>"))
                RW_THROW ("Unknown element \""
                          << child->getName ()
                          << "\" in ProximitySetup, should be Include or Exclude!");
        }
    }
    return setup;
}
...
DOMProximitySetupLoader::readFramePatternAttributes (DOMElem::Ptr element)
{
