...
std::vector< double > readNVector (DOMElement* element)
{
    ...
    if (len == 0) {
        ...
        RW_THROW ("Failed to parse element \""
                  ...
                  << "\". Too many child nodes. Nr children: " << children->getLength ());

    std::vector< double > elements =
        parseNArray (XMLStr (children->item (0)->getNodeValue ()).str ());
    return elements;
}
...
void checkHeader (DOMElement* element, const XMLCh* id)
{
    if (!XMLString::equals (element->getNodeName (), id))
        RW_THROW ("Expected \"" << XMLStr (id).str () << "\" got "
                                << XMLStr (element->getNodeName ()).str ());
}
...
Rotation3D<> XMLBasisTypes::readRotation3D (DOMElement* element, bool doCheckHeader)
{
    ...
    std::vector< double > values = readNVector (element);
    if (values.size () != 9)
        RW_THROW ("Expected 9 floating points for Rotation3D. Only " << values.size ()
                                                                     << " values found");
...
Rotation3D<> XMLBasisTypes::readRotation3DStructure (DOMElement* element)
{
    ...
    RW_THROW ("Unable to find match \"" << XMLStr (element->getNodeName ()).str ()
...
VelocityScrew6D<> XMLBasisTypes::readVelocityScrew6D (xercesc::DOMElement* element,
                                                      bool doCheckHeader)
{
    if (doCheckHeader)
    ...
    for (XMLSize_t i = 0; i < nodeCount; i++) {
        ...
        if (child != NULL) {
            if (XMLString::equals (child->getNodeName (), idLinear ())) {
                ...
                RW_THROW ("Unknown element \"" << XMLStr (child->getNodeName ()).str ()
...
std::vector< double > XMLBasisTypes::readDoubleList (xercesc::DOMElement* element,
                                                     bool doCheckHeader)
{
 ...
 */
rw::kinematics::State XMLBasisTypes::readState (xercesc::DOMElement* element,
                                                WorkCell::Ptr workcell, bool doCheckHeader)
{
    if (doCheckHeader)
...
const XMLCh* XMLBasisTypes::readElementTextXMLCh (xercesc::DOMElement* element,
                                                  bool exceptionOnEmpty)
{
    DOMNodeList* children = element->getChildNodes ();
...
double XMLBasisTypes::readDouble (xercesc::DOMElement* element, bool doCheckHeader)
{
    ...
    try {
        ...
        RW_WARN ("Double Value could not be parsed correctly form \""
                 << XMLStr (readElementTextXMLCh (element)).str () << "\". Setting to NaN! ");
...
namespace {
template< class T > XMLStr createStringFromArray (const T& v, size_t n)
{
...
template< class T > XMLStr createStringFromArray (const T& v)
{
    return createStringFromArray< T > (v, v.size ());
}

template< class T > XMLStr createStringFromVector (const T& v, size_t n)
{
    std::ostringstream str;
    str.unsetf (std::ios::floatfield);    // floatfield not set
    str.precision (16);
...
template< class T > XMLStr createStringFromVector (const T& v)
{
    return createStringFromVector< T > (v, v.size ());
}

}    // namespace
xercesc::DOMElement* XMLBasisTypes::createElement (const XMLCh* id, const XMLCh* value,
...
xercesc::DOMElement* XMLBasisTypes::createVector2D (const rw::math::Vector2D<>& v,
                                                    xercesc::DOMDocument* doc)
{
...
xercesc::DOMElement* XMLBasisTypes::createQuaternion (const rw::math::Quaternion<>& q,
                                                      xercesc::DOMDocument* doc)
{
...
DOMElement* XMLBasisTypes::createRotation3D (const rw::math::Rotation3D<>& r,
                                             xercesc::DOMDocument* doc)
{
...
xercesc::DOMElement* XMLBasisTypes::createRotation2D (const rw::math::Rotation2D<>& r,
                                                      xercesc::DOMDocument* doc)
{
...
xercesc::DOMElement* XMLBasisTypes::createTransform3D (const rw::math::Transform3D<>& t,
                                                       xercesc::DOMDocument* doc)
{
    xercesc::DOMElement* element = doc->createElement (idTransform3D ());

    xercesc::DOMElement* posElem =
        createElement (idPos (), createStringFromArray (t.P ()).uni (), doc);
...
DOMElement* XMLBasisTypes::createVelocityScrew6D (const rw::math::VelocityScrew6D<>& v,
                                                  xercesc::DOMDocument* doc)
{
...
xercesc::DOMElement* XMLBasisTypes::createIntList (const std::vector< int >& ints,
                                                   xercesc::DOMDocument* doc)
{
    return createElement (idIntList (), createStringFromVector (ints).uni (), doc);
}

xercesc::DOMElement* XMLBasisTypes::createDoubleList (const std::vector< double >& doubles,
                                                      xercesc::DOMDocument* doc)
{
    return createElement (idDoubleList (), createStringFromVector (doubles).uni (), doc);
}

xercesc::DOMElement* XMLBasisTypes::createQState (const rw::kinematics::State& state,
                                                  xercesc::DOMDocument* doc)
{
    return createElement (
        idQState (), createStringFromArray< State > (state, state.size ()).uni (), doc);
...
xercesc::DOMElement* XMLBasisTypes::createStringList (const std::vector< std::string >& strings,
                                                      xercesc::DOMDocument* doc)
{
...
xercesc::DOMElement* XMLBasisTypes::createStringPair (const std::string& first,
...
xercesc::DOMElement* XMLBasisTypes::createTreeState (const rw::kinematics::State& state,
                                                     xercesc::DOMDocument* doc)
{
...
xercesc::DOMElement* XMLBasisTypes::createState (const rw::kinematics::State& state,
                                                 xercesc::DOMDocument* doc)
{
