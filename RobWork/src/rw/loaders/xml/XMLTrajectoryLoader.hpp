...
namespace rw { namespace loaders {
     ...
     *
     * The XMLTrajectoryLoader loads in a file containing a trajectory specified according to the
     ...
     * be determined using the XMLTrajectoryLoader::getType after loading.
     *
    ...
    class XMLTrajectoryLoader
    {
      public:
         ...
         *
         * If the XMLTrajectoryLoader is used outside main (as a part of global
         ...
         * should not be necessary to specify the initializer explicitly if XMLTrajectoryLoader is
        ...
        /* template<class T>
         void initialize(T t, const std::string& schemaFileName) {
             ...
             if (parser.getErrorCount() != 0) {
                 std::cerr<<std::endl<<std::endl<<"Error(s) =
         "<<std::endl<<XMLStr(errorHandler.getMessages()).str()<<std::endl;
                 RW_THROW(""<<parser.getErrorCount()<<" Errors:
         "<<XMLStr(errorHandler.getMessages()).str());
             }
