...
ThreadTask::ThreadTask (ThreadTask::Ptr parent) :
    ...
    _children (new ThreadSafeVariable< std::vector< ThreadTask::Ptr > > (
        std::vector< ThreadTask::Ptr > ())),
...
bool ThreadTask::setThreadPool (ThreadPool::Ptr pool)
{
    ...
    // (blocks execute() from moving from INITILIZATION state and launching the job before pool has
    // been set)
...
bool ThreadTask::execute ()
{
    ...
    //  - note that a subtask added at this point might actually start executing before we add this
    //  task to the pool
    // The ThreadPool can not change anymore (state is IN_QUEUE) so we just read it without holding
    // the lock:
    ...
    for (ThreadTask::Ptr subtask : children) {
        subtask->execute ();
    }
    return true;
}
...
void ThreadTask::setKeepAlive (bool keepAlive)
{
    ...
    {
        // Lock mutex to get exclusive access to change private member variables.
        // This is done to be sure that the state does not change to and from IDLE while keepAlive
        // is changing this influences if finish() should be invoked or not.
        boost::mutex::scoped_lock lock (_mutex);
        // If keepAlive is switched off and the task is IDLE, we finish it.
        if (_keepAlive->getVariable () && !keepAlive) {
            if (_state->getVariable () == IDLE) {
                if (!_blockFinalize->getVariable ()) {    // only finish task if no subtasks are
                                                          // currently invoking idle
...
void ThreadTask::runWrap (ThreadPool* pool)
{
    ...
    try {
        ...
        RW_THROW (
            "Please catch boost::thread_interrupted exception thrown in ThreadTask run funtion!");
    }
    // State is set to CHILDREN causing children to try to move to IDLE when they finish (if keep
    // alive is disabled).
    {
        boost::mutex::scoped_lock lock (_mutex);
        _state->setVariable (CHILDREN);
    ...
    bool doIdle = false;
    {
        ...
        if (_childrenMissing->getVariable () == 0) {
            if (_state->getVariable () == CHILDREN) {
                ...
                doIdle = true;
                _blockFinalize->setVariable (
                    true);    // to make sure that setKeepAlive(false) will not finish task to early
            }
        }
    }
    bool doFinish = false;
            ...
            if (_childrenMissing->getVariable () == 0) {
                if (_state->getVariable () == IDLE) {
                    if (!_keepAlive->getVariable ()) {
                        _state->setVariable (
                            POSTWORK);    // addSubTasks() can not be used any longer
                        doFinish = true;
                    }
                }
            }
...
void ThreadTask::finish ()
{
    ...
    try {
        ...
        RW_THROW (
            "Please catch boost::thread_interrupted exception thrown in ThreadTask done funtion!");
    }
