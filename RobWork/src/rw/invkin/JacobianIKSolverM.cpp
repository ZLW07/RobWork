...
JacobianIKSolverM::JacobianIKSolverM (const TreeDevice* device, const State& state) :
    IterativeMultiIK (device->getEnds ().size ()), _device (device), _foi (device->getEnds ()),
    _fkranges (createFKRanges (device->getBase (), _foi, state)), _interpolationStep (0.2),
    _returnBestFit (false), _useJointClamping (true), _useInterpolation (true),
...
JacobianIKSolverM::JacobianIKSolverM (const JointDevice* device, const std::vector< Frame* >& foi,
                                      const State& state) :
    IterativeMultiIK (foi.size ()),
    _device (device), _foi (foi), _fkranges (createFKRanges (device->getBase (), _foi, state)),
    _interpolationStep (0.2), _returnBestFit (false), _useJointClamping (true),
    _useInterpolation (true), _solverType (JacobianIKSolverM::SVD)
{
    setMaxIterations (40);
    _jacCalc = device->baseJCframes (foi, state);
}

bool JacobianIKSolverM::solveLocal (const std::vector< Transform3D<> >& bTed,
                                    std::vector< double >& maxError, State& state, int maxIter,
...
std::vector< Q > JacobianIKSolverM::solve (const std::vector< Transform3D<> >& bTeds,
    ...
    if (_useInterpolation) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            interpolators.push_back (
                LinearInterpolator< Transform3D<> > (bTeInit, bTeds[i], _interpolationStep));
        }

        std::vector< Transform3D<> > bTeds_via (N);
        for (double t = _interpolationStep; t < interpolators[0].duration ();
             t += _interpolationStep) {
            // calculate the bTes
            for (int i = 0; i < N; i++)
