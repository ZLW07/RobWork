...
namespace rw { namespace proximity {
    ...
    class OBVTreeDFSCollider : public rw::proximity::BVTreeCollider< BVTREE >
    {
      ...
      public:
        ...
        OBVTreeDFSCollider (BVCOLLIDER* bvcollider, PRIMCOLLIDER* primcollider,
                            DESCENTSTRATEGY* descendStrat, int n = 200) :
            _bvCollider (bvcollider),
            _primCollider (primcollider), _descendStrat (descendStrat), _BVstack (n),
            _BVstackIdx (0), _firstContact (true)
        {
            initVars ();
        }
        ...
        bool collides (const rw::math::Transform3D< value_type >& fTA, const BVTREE& treeA,
      ...
      private:
        ...
        inline bool empty () const { return _BVstackIdx == 0; }
        inline void push ()
        {
      ...
      private:
        struct Job
        {
          public:
            ...
            Job (NodeIterator tA, NodeIterator tB,
    ...
    bool OBVTreeDFSCollider< BVTREE, BVCOLLIDER, DESCENTSTRATEGY, PRIMCOLLIDER >::collides (
        const rw::math::Transform3D<
            typename Traits< typename Traits< BVTREE >::BVType >::value_type >& fTA,
        const BVTREE& treeA,
        const rw::math::Transform3D<
            typename Traits< typename Traits< BVTREE >::BVType >::value_type >& fTB,
        const BVTREE& treeB, std::vector< std::pair< int, int > >* collidingPrimitives)
    {
        typedef typename Traits< BVTREE >::NodeIterator NodeIterator;
        typedef typename Traits< BVTREE >::BVType BVType;
        ...
        typename Traits< DESCENTSTRATEGY >::StateType descendState =
            typename Traits< DESCENTSTRATEGY >::StateType ();
        ...
        while (!empty ()) {
            ...
            // "\t" << job.nodeA.getId() << " -- " << job.nodeB.getId() <<" [style=dotted]; \n";
            ...
            if (isALeaf && isBLeaf) {
                ...
                if ((descentA && !job.nodeA.isLeaf ()) || job.nodeB.isLeaf ()) {
                    // TODO: optimize such that only 1 is pushed on stack, the other is kept in
                    // local variables
