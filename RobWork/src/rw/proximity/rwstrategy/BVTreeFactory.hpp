...
namespace rw { namespace proximity {
    ...
    class BVTreeFactory
    {
      public:
         ...
         */
        template< class BV > struct BVSplitterStrategy
        {
            virtual size_t partitionMesh (rw::geometry::IndexedTriArray<>& mesh,
                                          const BV& bv) const = 0;
        };
         ...
         */
        template< class BV > struct BVShellSplitterStrategy
        {
            /**
        ...
        struct TriMeshAccessor : public PrimArrayAccessor< rw::geometry::Triangle< T > >
        {
            ...
            size_t getSize () const { return _mesh->getSize (); }
        ...
        struct ShellAccessor : public PrimArrayAccessor< rw::geometry::GenericFace >
        {
            ...
            size_t getSize () const { return _object->size (); }
        ...
        /**
         * @brief creates an OBB tree using a covariance OBB factory and a object median splitting
         * strategy.
        ...
        template< class BVTREE >
        BVTREE* makeTopDownOBBTreeCovarSpatialMedian (rw::geometry::TriMesh::Ptr mesh,
                                                      int maxTrisInLeaf = 1)
        {
            typedef typename Traits< BVTREE >::BVType BVType;
            rw::core::Ptr< geometry::BVFactory< typename Traits< BVTREE >::BVType > > bvfactory =
        ...
        template< class BVTREE >
        BVTREE* makeTopDownOBBIdxTreeCovarMedian (rw::geometry::TriMesh::Ptr mesh,
                                                  int maxTrisInLeaf = 1)
        {
            typedef typename Traits< BVTREE >::BVType BVType;
            rw::core::Ptr< geometry::BVFactory< typename Traits< BVTREE >::BVType > > bvfactory =
        ...
        /**
         * @brief general function for constructing a binary bounding volume tree in a top down
         * fashion.
         *
         * @param mesh [in] the mesh on which to construct the bounding volume tree
         * @param bvFactory [in] a factory for creating/fitting bounding volumes given a triangle
         * mesh
         * @param splitter [in] divides a mesh into 2 meshes by sorting the mesh and providing a
         * splitting index
         * @param maxTrisInLeaf [in] the maximum number of tris that are allowed in each leaf node
         * @return
         */
        template< class BVTREE >
        static BVTREE*
        makeTopDownTree (rw::geometry::TriMesh::Ptr mesh,
                         ...
                         int maxTrisInLeaf = 1)
        {
            ...
            recursiveTopDownTree< BVTREE > (
                tree, root, idxArray, bvFactory, splitter, maxTrisInLeaf);
         ...
         */
        template< class BVTREE >
        static BVTREE* makeTopDownTree (
            rw::geometry::Shell::CPtr shell,
            geometry::BVFactory< typename Traits< BVTREE >::BVType >& bvFactory,
         ...
         */
        template< class BVTREE >
        static void recursiveTopDownTree (
            BVTree< BVTREE >* tree, typename Traits< BVTREE >::NodeIterator& node,
            ...
            size_t maxTrisInLeaf)
        {
            ...
            if (mesh.getSize () == 0) {
                ...
                BVType bv = bvFactory.makeBV (mesh);
                tree->setBV (bv, node);
                // std::cout << "Range: "<< mesh.getGlobalIndex(0) << ";" <<
                // mesh.getGlobalIndex(mesh.getSize()) << std::endl;
                ...
                if (k > 0) {
                    ...
                    recursiveTopDownTree (tree,
                                          leftnode,
                                          mesh.getSubRange (0, k),
                                          bvFactory,
                                          splitter,
                                          maxTrisInLeaf);
                }
                ...
                if (k < mesh.getSize ()) {
                    ...
                    recursiveTopDownTree (tree,
                                          rightnode,
                                          mesh.getSubRange (k, mesh.getSize ()),
                                          bvFactory,
                                          splitter,
                                          maxTrisInLeaf);
                }
        ...
        /**
         * @brief computes a score of a specific splitting axis. Basically each triangle in the mesh
         * is visited and all vertices are tested for being on either left or right side of the
         * splitting value. The score is then the total number of triangles being completely on
         * either left or right side. Which means that a score=mesh.size() is the best score where
         * the split value completely splits the triangles into two volumes. score=0 is the worst
         * where all triangles has vertices on both sides on the split value.
         ...
         */
        template< class T >
        static int evaluateSplitAxis (rw::geometry::IndexedTriArray<>& mesh, int splitAxis,
                                      double splitValue, const rw::math::Transform3D< T >& t3d)
        {
            using namespace rw::geometry;
        ...
        struct OBVMedianSplitter : public BVTreeFactory::BVSplitterStrategy< BV >
        {
          public:
            ...
            size_t partitionMesh (rw::geometry::IndexedTriArray<>& mesh, const BV& obb) const
            {
                ...
                int splitAxis      = 0,
                    bestSplitAxis  = 0;    // choose longest (x-axis) for splitting the Box
                ...
                do {
                    ...
                    Vector3D<> center =
                        t3d * ((tri.getVertex (0) + tri.getVertex (1) + tri.getVertex (2)) / 3.0);
                    // t.reset();
            ...
            size_t partitionMesh (rw::geometry::IndexedTriArray<>& mesh, const BV& obb) const
            {
                ...
                do {
                    ...
                    for (size_t i = 0; i < mesh.getSize (); i++) {
                        ...
                        Vector3D<> center =
                            t3d *
                            ((tri.getVertex (0) + tri.getVertex (1) + tri.getVertex (2)) / 3.0);
