...
/*class BalancedTreeDistanceCalc : public TreeDistanceCalc
{
  ...
  public:
    typedef BinaryBVTree< rw::geometry::OBB<>, rw::geometry::Triangle<> >::NodeIterator
        NodeIterator;
    // typedef ::rw::Traits<BinaryBVTree<rw::geometry::OBB<>, rw::geometry::Triangle<> >
    ...
    BalancedTreeDistanceCalc (rw::core::Ptr< TreeDistanceCalc::BVDISTANCECALC > bvdistcalc,
                              int n = 200) :
        _bvdistcalc (bvdistcalc),
        _BVstack (n), _BVstackIdx (0)
    {
        initVars ();
    }
    ...
    double distance (const rw::math::Transform3D<>& fTA, const TreeDistanceCalc::BVTree& cmA,
    ...
    int getMemUsage () { return _BVstackIdx * sizeof (BVJob); }
  ...
  private:
    struct BVJob
    {
      public:
        ...
        BVJob (
            const BinaryBVTree< rw::geometry::OBB<>, rw::geometry::Triangle<> >::NodeIterator& tA,
            const BinaryBVTree< rw::geometry::OBB<>, rw::geometry::Triangle<> >::NodeIterator& tB,
            int ai, int bi, bool isAA = true) :
            treeA (tA),
            treeB (tB), isAfromA (isAA), aIdx (ai), bIdx (bi)
        {}
