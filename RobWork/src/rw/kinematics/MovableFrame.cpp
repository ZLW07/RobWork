...
MovableFrame::MovableFrame (const std::string& name) : Frame (12, name)
{}

void MovableFrame::doMultiplyTransform (const Transform3D<>& parent, const State& state,
...
Transform3D<> MovableFrame::doGetTransform (const State& state) const
{
    ...
    const Rotation3D<> rot (q[3], q[4], q[5], q[6], q[7], q[8], q[9], q[10], q[11]);
    // quat.normalize();
