...
void IOUtil::readFile (const std::string& file_name, std::vector< char >& result)
{
...
namespace {
std::string intersperse (const std::vector< std::string >& vals, const std::string& sep)
{
...
std::string resolveSuffix (const std::string& raw_filename, const std::string& suffix)
{
    ...
    return isReadable (v1) ? v1 : isReadable (v2) ? v2 : isReadable (v3) ? v3 : "";
}

std::string resolveFileNameHelper (const std::string& raw_filename,
                                     ...
                                     const std::vector< std::string >& extensions)
{
    ...
    const std::string& file = resolveFileNameHelper (raw_filename, extensions);
    ...
    if (!file.empty ()) {
        ...
        RW_THROW ("Can't open file "
                  ...
                  << ". Optional extensions considered: " << intersperse (extensions, " "));
...
std::string IOUtil::getAbsoluteFileName (const std::string& file)
{
    ...
    char* res = NULL;
    // Previously this was called without the _. This however should be more compliant with ISO C++
...
void IOUtil::getFilesInFolder (const std::string& path, const std::string& fileMask, bool recursive,
                               bool addPath, std::vector< std::string >& result)
{
    try {
...
std::vector< std::string > IOUtil::getFilesInFolder (const std::string& path, bool recursive,
                                                     bool addPath, const std::string& fileMask)
{
...
std::string IOUtil::getFirstXMLElement (const std::string& filename)
{
    ...
    parse_info< file_iterator< char > > info = parse (
        first,
        last,
        !("<?" >> *(anychar_p - '>')    //[std::cout << boost::lambda::_1]
          >> '>') >>
            "<"
            //>> *( (anychar_p - '>') | (anychar_p -' '))[boost::bind(static_cast<std::string&
            //(std::string::*)( size_t, char )>(&std::string::append),&result, 1, ::_1)]
            >> *((anychar_p - '>') |
                 (anychar_p - ' '))[var (result) = construct_< std::string > (arg1, arg2)] >>
            *(anychar_p - '>')    //[std::cout << boost::lambda::_1]
            >> ">"
        // THE skip parser comes next
        ,
        (blank_p - ' ') | (space_p - ' ') | ("<!--" >> *(anychar_p - '>') >> '>')
    ...
    std::string result;
    // use iterator to parse file data
    bool res = spirit::qi::phrase_parse (
        begin,
        end,
        -("<?" >> *(char_ - '>') /*[std::cout << boost::lambda::_1]*/ >> char_ ('>')) >> "<" >>
            *(char_ - (char_ ('>')))[boost::bind (
        ...
        // THE skip parser comes next
        ,
        spirit::qi::blank | spirit::qi::space | ("<!--" >> *(char_ - '>') >> '>')
...
#warning Your boost version is not new enough
    ...
    if (!res)
        RW_THROW ("The provided ifstream does not consist of a wellformed xml document! "
                  << result);
    return result;
#endif
}
