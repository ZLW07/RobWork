...
namespace rw { namespace graphics {
    ...
    class SceneViewer
    {
      public:
         ...
         */
        virtual void saveBufferToFile (const std::string& filename, const int fillR,
                                       const int fillG, const int fillB) = 0;
        ...
        struct View
        {
            ...
            View (const std::string& name) :
                _name (name), _drawType (DrawableNode::SOLID),
        ...
        /* A view always has one camera attached, this is the getSceneCamera(). Besides that a
         * number of slave cameras can be attached. These are manipulated through
         * getSlaveSceneCamera()
         */
         ...
         *
         * To send an event double clicking left mouse button on the point of interest while
         * pressing shift.
         *
        ...
        {
            return getViewCamera ()->getTransform ();
        };
