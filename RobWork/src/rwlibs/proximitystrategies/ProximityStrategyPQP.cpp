...
void toRapidTransform (const Transform3D<>& tr, PQP_REAL R[3][3], PQP_REAL T[3])
{
...
void pqpCollide (PQP_Model& ma, const Transform3D<>& wTa, PQP_Model& mb, const Transform3D<>& wTb,
                 PQP_CollideResult& result, bool firstContact)
{
...
void pqpTolerance (PQP_Model& ma, const Transform3D<>& wTa, PQP_Model& mb, const Transform3D<>& wTb,
                   double tolerance, PQP_ToleranceResult& result)
{
    PQP_REAL ra[3][3], rb[3][3], ta[3], tb[3];
...
void pqpDistance (PQP_Model* ma, const Transform3D<>& wTa, PQP_Model* mb, const Transform3D<>& wTb,
                  double rel_err, double abs_err, PQP_DistanceResult& result)
{
    PQP_REAL ra[3][3], rb[3][3], ta[3], tb[3];
...
void pqpMultiDistance (double threshold, PQP_Model* ma, const Transform3D<>& wTa, PQP_Model* mb,
                       const Transform3D<>& wTb, double rel_err, double abs_err,
                       PQP_MultiDistanceResult& result)
{
    ...
    PQP_DistanceMultiThreshold (&result,
                                (PQP_REAL) threshold,
                                ra,
                                ta,
                                ma,
...
void pqpDistanceThreshold (PQP_Model* ma, const Transform3D<>& wTa, PQP_Model* mb,
                           const Transform3D<>& wTb, double threshold, double rel_err,
                           double abs_err, PQP_DistanceResult& result)
{
    PQP_REAL ra[3][3], rb[3][3], ta[3], tb[3];
                                        ...
                                        rw::geometry::Geometry::Ptr geom, bool forceCopy)
{
    ...
    RWPQPModel rwpqpmodel (geom, geom->getTransform (), pqpmodel);
    rwpqpmodel.ckey = key;
...
{
    RW_WARN (
        "THIS METHOD ProximityStrategyPQP::setFirstContact IS DEPRECATED AND WILL HAVE NO EFFECT!");
    //_firstContact = b;
}

ProximityStrategyPQP::QueryData ProximityStrategyPQP::initQuery (ProximityModel::Ptr& aModel,
...
bool ProximityStrategyPQP::doIsWithinDistance (ProximityModel::Ptr aModel, const Transform3D<>& wTa,
...
bool ProximityStrategyPQP::doInCollision (ProximityModel::Ptr aModel, const Transform3D<>& wTa,
                                          ProximityModel::Ptr bModel, const Transform3D<>& wTb,
    ...
    for (const RWPQPModel& ma : qdata.a->models) {
        for (const RWPQPModel& mb : qdata.b->models) {
            ...
            if (qdata.cache->_collideResult.Colliding () != 0) {
                // data._aTb =
                // fromRapidTransform(qdata.cache->_collideResult.R,qdata.cache->_collideResult.T);
                ...
                for (int j = 0; j < size; j++) {
                    data._geomPrimIds[startIdx + j].first =
                        qdata.cache->_collideResult.pairs[j].id1;
                    data._geomPrimIds[startIdx + j].second =
                        qdata.cache->_collideResult.pairs[j].id2;
                }
                if (firstContact)
                    return true;
...
DistanceResult& ProximityStrategyPQP::doDistance (ProximityModel::Ptr aModel,
...
MultiDistanceResult& ProximityStrategyPQP::doDistances (ProximityModel::Ptr aModel,
                                                        const Transform3D<>& wTa,
                                                        ProximityModel::Ptr bModel,
...
ProximityStrategyPQP::getGeometryIDs (rw::proximity::ProximityModel* model)
{
...
DistanceResult&
ProximityStrategyPQP::doDistanceThreshold (ProximityModel::Ptr aModel, const Transform3D<>& wTa,
                                           ProximityModel::Ptr bModel, const Transform3D<>& wTb,
    ...
    std::vector< CollisionStrategy::Contact >& contacts, rw::proximity::ProximityStrategyData& data)
{
    ...
    for (CollisionStrategy::Result::CollisionPair& cpair : res._collisionPairs) {
        ...
        for (int i = cpair.startIdx; i < cpair.startIdx + cpair.size; i++) {
            ...
            Triangle<> triA (
                fromRapidVector (atri->p1), fromRapidVector (atri->p2), fromRapidVector (atri->p3));
            Triangle<> triB (res._aTb * fromRapidVector (btri->p1),
                             res._aTb * fromRapidVector (btri->p2),
                             res._aTb * fromRapidVector (btri->p3));
