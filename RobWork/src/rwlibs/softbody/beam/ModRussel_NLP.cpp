...
ModRussel_NLP::ModRussel_NLP (std::shared_ptr< rwlibs::softbody::BeamGeometry > geomPtr,
    ...
    _geomPtr (geomPtr),
    _obstaclePtr (obstaclePtr), _planeTbeam (planeTbeam), _integralIndices (integralIndices),
    _Ee (0.0)
{
...
bool ModRussel_NLP::get_nlp_info (Ipopt::Index& n, Ipopt::Index& m, Ipopt::Index& nnz_jac_g,
...
bool ModRussel_NLP::get_bounds_info (Ipopt::Index n, Ipopt::Number* x_l, Ipopt::Number* x_u,
...
bool ModRussel_NLP::get_starting_point (Ipopt::Index n, bool init_x, Ipopt::Number* x, bool init_z,
...
bool ModRussel_NLP::eval_f (Ipopt::Index n, const Ipopt::Number* x, bool new_x,
...
bool ModRussel_NLP::eval_grad_f (Ipopt::Index n, const Ipopt::Number* x, bool new_x,
                                 Ipopt::Number* grad_f)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        eval_f (n,
                xt,
                new_x,
                d1);    // evaluate function value for negative pertubation of the i'th variable
        ...
        eval_f (n,
                xt,
                new_x,
                d2);    // evaluate function value for positive pertubation of the i'th variable

        grad_f[i] =
            (d2 - d1) / (2 * eps);    // centered finite-difference calculation of the first partial
...
bool ModRussel_NLP::eval_g (Ipopt::Index n, const Ipopt::Number* x, bool new_x, Ipopt::Index m,
                            Ipopt::Number* g)
{
    int gBase = 0;    // the base offset into the vector of constraints passed to IPOPT

    // go through the list of indices into the beam at which to set a no-penetration constraint
    // value
...
bool ModRussel_NLP::eval_jac_g (Ipopt::Index n, const Ipopt::Number* x, bool new_x, Ipopt::Index m,
...
// evaluates the constraint value of the point at index pIdx on the beam, placing it in the vector
// g[gBase]
void ModRussel_NLP::eval_g_point (int pIdx, int gBase, Index n, const Number* x, bool new_x,
                                  Index m, Number* g)
{
    const rw::math::Transform3D<> planeTbeam = get_planeTbeam ();    //
    ...
    //     double resU = ( ( hx / 2.0 ) * ( f0U + fLU ) + hx * sumU)     +          0.5 * 7.0 * sin(
    //     x[pIdx] ); // HACK beam thickness hardcoded double resU = ( ( hx / 2.0 ) * ( f0U + fLU )
    //     + hx * sumU)     +          (0.5 * dx * cos( x[pIdx] ) - 0.5 *dy * sin(x[pIdx]));
    ...
    //     double resV = ( ( hx / 2.0 ) * ( f0V + fLV ) + hx * sumV )     -          0.5 * 7.0 *
    //     cos( x[pIdx] ); // HACK beam thickness hardcoded
    double resV =
        ((hx / 2.0) * (f0V + fLV) + hx * sumV) +
        (-0.5 * dy * cos (x[pIdx]) - 0.5 * dx * sin (x[pIdx]));    // HACK beam thickness hardcoded
    //     double resV = ( ( hx / 2.0 ) * ( f0V + fLV ) + hx * sumV );

    g[gBase] = resU * uxTCPy + resV * uyTCPy;    // require h(x) > 0
}

// evaluates the value of constraint jacobian of the point at index pIdx on the beam, placing it in
// the constraint jacobian with row gBase
void ModRussel_NLP::eval_jac_g_point (int pIdx, int gBase, Index n, const Number* x, bool new_x,
                                      ...
                                      Number* values)
{
    ...
    // populate the mxn jacobian of the constraint functions (eg. 1x31 with one integral constraint
    // and 32 slices)
    if (values == NULL) {
        ...
        for (int j = 0; j < n; j++) {       // loop through all the columns (refering to each x[j]
            iRow[gBase * n + j] = gBase;    // the constraint index (e.g. g[0]) is gBase and is the
                                            // row in which to place it
        ...
        for (int j = 0; j < n; j++) {
            if (pIdx == j) {
                //                 values[gBase*n + j] = 0.5 * hx * uyTCPy * cos ( x[j] ) - 0.5 * hx
                //                 * uxTCPy * sin ( x[j] );
                // HACK beam thickness hardcoded
                values[gBase * n + j] =
                    ((-0.5 * hx * sin (x[j]))) * uxTCPy +    // U part
                    ((0.5 * hx * cos (x[j])) + (-0.5 * dx * cos (x[j]) + 0.5 * dy * sin (x[j]))) *
                        uyTCPy;

                //(  (-0.5 * hx * sin (x[j]) )  +  (-0.5*dy*cos(x[j]) - 0.5*dx*sin(x[j]))   ) *
                //uxTCPy +        // U part
            }
            else if (j < pIdx) {
...
//
// currently we just let IPOPT calculate this numerically by having set the options to (
// "hessian_approximation", "limited-memory" )
bool ModRussel_NLP::eval_h (Ipopt::Index n, const Ipopt::Number* x, bool new_x,
...
void ModRussel_NLP::finalize_solution (Ipopt::SolverReturn status, Ipopt::Index n,
