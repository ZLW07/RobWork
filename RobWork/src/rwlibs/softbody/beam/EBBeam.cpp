...
EBBeam::EBBeam (const double H, const double K, const double L, const double E, const double rho,
                const double h, const double g2) :
    _H (H),
    _K (K), _L (L), _E (E), _rho (rho), _h (h)
{
...
double EBBeam::operator() (const int i) const
{
    ...
    return (_q * pow (x, 2.0) * (6.0 * pow (_L, 2.0) - 4.0 * _L * x + pow (x, 2.0))) /
           (24.0 * _E * _J);
};
