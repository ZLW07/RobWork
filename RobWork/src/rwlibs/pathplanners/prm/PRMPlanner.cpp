...
PRMPlanner::PRMPlanner (rw::pathplanning::QConstraint::Ptr constraint,
                        rw::pathplanning::QSampler::Ptr sampler, double resolution,
                        const rw::models::Device& device, const rw::kinematics::State& state) :
    _constraint (constraint),
    _sampler (sampler), _resolution (resolution),
    _edge (QEdgeConstraint::make (_constraint, MetricFactory::makeEuclidean< Q > (), resolution))
{
    initialize (device, state);
}

PRMPlanner::PRMPlanner (rw::models::Device* device, const rw::kinematics::State& state,
                        rw::proximity::CollisionDetector* detector, double resolution) :
    _constraint (QConstraint::make (detector, device, state)),
    _sampler (QSampler::makeUniform (*device)), _resolution (resolution),
    _edge (QEdgeConstraint::make (_constraint, MetricFactory::makeEuclidean< Q > (), resolution))
{
    initialize (*device, state);
}

void PRMPlanner::initialize (const Device& device, const State& state)
{
    _bounds = device.getBounds ();

    _metricWeights = PlannerUtil::estimateMotionWeights (
        device, device.getEnd (), state, PlannerUtil::WORSTCASE, 1000);
...
{
    return _constraint->inCollision (a) || _constraint->inCollision (b) ||
...
void PRMPlanner::addEdges (Node node)
{
    ...
    if (_neighborSearchStrategy == BRUTE_FORCE) {
        ...
        for (size_t i = 0; i < radi.size (); i++) {
            radi[i] = _Rneighbor / std::max (_metricWeights (i),
                                             0.1);    // scale the neighbor according to weights
        }
 ...
 */
bool PRMPlanner::doQuery (const rw::math::Q& qInit, const rw::math::Q& qGoal, QPath& path,
                          const StopCriteria& stop)
{
    ...
    if (qInit.size () != _bounds.first.size ())
        RW_THROW ("qInit input is not the right dimension. " << qInit.size ()
                                                             << "!=" << _bounds.first.size ());
    if (qGoal.size () != _bounds.first.size ())
        RW_THROW ("qGoal input is not the right dimension. " << qGoal.size ()
                                                             << "!=" << _bounds.first.size ());
    ...
    if (!_roadmap_initialized) {
        RW_WARN ("Roadmap was not build/initialized before first query. It will be built now with "
                 "1000 nodes!");
        buildRoadmap (1000);
    }
    ...
    while (!stop.stop ()) {
        ...
        switch (_shortestPathSearchStrategy) {
            case A_STAR: pathFound = searchForShortestPathAstar (nInit, nGoal, nodepath); break;
            case DIJKSTRA:
...
bool PRMPlanner::inCollision (std::list< Node >& path)
{
    ...
    if (_collisionCheckingStrategy == LAZY) {
        for (int i = 0; i < (int) path.size (); i++) {
            ...
            int index1 = (int) std::floor ((double) i / 2.0) * (-(i % 2) + (i + 1) % 2) +
                         (i % 2) * ((int) path.size () - 1);
    ...
    //    std::priority_queue<Segment, std::vector<Segment>, SegmentCompare >
    //    edgeQueue(SegmentCompare(&_graph), std::vector<Segment>(0));
...
bool PRMPlanner::searchForShortestPathDijkstra (const Node& nInit, const Node& nGoal,
                                                std::list< Node >& result)
{
    ...
    boost::dijkstra_shortest_paths (
        _graph,
        nInit,
        boost::predecessor_map (p).vertex_index_map (indexMap).weight_map (
            get (&EdgeData::weight, _graph)));
 ...
 */
template< class Node > class AStarGoalVisitor : public boost::default_astar_visitor
{
  public:
    /**
    ...
    AStarGoalVisitor (Node nGoal, double timeoutTime) :
        _nGoal (nGoal), _astarTimeOutTime (timeoutTime)
    {
     ...
     */
    template< class Graph > void examine_vertex (Node n, Graph& g)
    {
        if (n == _nGoal)
            throw FoundGoal ();
...
bool PRMPlanner::searchForShortestPathAstar (const Node& nInit, const Node& nGoal,
                                             std::list< Node >& result)
{
    ...
    try {
        ...
        // const boost::bundle_property_map<PRM,
        // boost::detail::edge_desc_impl<boost::undirected_tag, void*>,
        // rwlibs::pathplanners::PRMPlanner::EdgeData, double> 	&pmap = get(&EdgeData::weight,
        //_graph);
...
void PRMPlanner::setShortestPathSearchStrategy (
