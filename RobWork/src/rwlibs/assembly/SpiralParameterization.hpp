...
namespace rwlibs { namespace assembly {
     ...
     * The blue point shows the initial location of the one end of the peg, while the red spiral
     * shows the opposite end of the peg. During the spiral motion, the peg will gradually go from
     * vertical to more and more horizontal. At the same time, the one end of the peg will remain on
     * the same vertical line compared to the hole.
     *
     ...
     * --------------------------------------------------------------------------- |
     * ------------------- | -------------------- | | r                 | The spiral radius. |
     * \f$\frac{m}{rad}\f$ | \f$\frac{mm}{rad}\f$ | | n                 | Number of turns. | | | |
     * \f$l_{peg}\f$     | Length from end of peg to the point where rotation occurs (the blue
     * point). | m                   | mm                   | | \f$l_{push}\f$    | Offset of the
     * peg in the z-direction.                                       | m                   | mm | |
     * \f$l_{start}\f$   | Offset of the peg in the z-direction for the approach pose. | m | mm | |
     * \f$d_{success}\f$ | Depth in hole where insertion is considered successful. | m | mm | | v |
     * Speed of motion.                                                            |
     * \f$\frac{m}{s}\f$   | \f$\frac{mm}{s}\f$   | | \f$\Delta d\f$    | Discretization of spiral.
     * | rad                 | rad                  | | \f$f_{max}\f$     | Maximum force allowed
     * before strategy is aborted.                           | N                   | N |
     */
    class SpiralParameterization : public AssemblyParameterization
    {
      public:
         ...
         * @param length_peg [in] length from end of peg to the point where the rotation occurs (in
         * meters).
         * @param length_push [in] offset of the peg in the z-direction (in meters).
         * @param length_start_traj [in] offset of the peg in the z-direction for the approach pose
         * (in meters).
         * @param depth_in_hole_for_success [in] depth in hole where insertion is considered
         * successful (in meters).
      ...
      public:
        ...
        double d_path;
        //! @brief The strategy aborts if the force sensor measures a higher force than this maximum
        //! allowed force.
