...
namespace rwlibs { namespace algorithms {
     ...
     *
     * Models a stable pose with 1 DoF, i.e. a set of rotations allowed for object with 1 axis of
     * rotation possible (e.g. a box on a surface). For a case of coordinate frame rotating around
     * single arbitrary axis, the points at the ends of x, y, and z versors fall into parallel
     * planes on a unit sphere. A stable pose with 1 DoF freedom is represented by a normal vector
     * for the planes, and distances Dx, Dy, and Dz to respective planes, measured from the point
     * {0, 0, 0}. For some cases, only two planes are defined, omitting the third coordinate (NaN).
     * If only one plane was found, the model is considered invalid.
     *
    ...
    class StablePose1DModel : public RANSACModel< StablePose1DModel, rw::math::Rotation3D<> >
    {
      ...
      public:    // constructors
         ...
         * reference, and a placement of that axis in global frame of reference.
         *
      ...
      public:    // methods
         ...
         *
         * Fit error for the stable pose is calculated as norm2 of distances of its x, y and z
