...
rw::math::Transform3D<>
PointPairsRegistration::pointPairRegistrationQuaternion (const std::vector< PointPair >& pointPairs)
{
    ...
    // The Eigen vector of Q corresponding to the largest Eigen value gives the quaternion, q,
    // representing the optimal rotation.
    std::pair< typename Eigen::MatrixXcd, typename Eigen::VectorXcd > eigenDecomp =
        LinearAlgebra::eigenDecomposition (Q);

    Quaternion<> qua (eigenDecomp.first (1, 0).real (),
                      eigenDecomp.first (2, 0).real (),
                      eigenDecomp.first (3, 0).real (),
                      eigenDecomp.first (0, 0).real ());
...
rw::math::Transform3D<>
PointPairsRegistration::pointPairRegistrationSVD (const std::vector< PointPair >& pointPairs)
{
