...
/*
    ...
    public:
        ...
        KinnectSensorWrapper(SimulatedScanner25D *scanner,rw::kinematics::Frame *sframe, const
   std::string& name): Scanner25D(name), _simscanner(scanner)
        {
            attachTo(sframe);
        }
...
class KinectSensorModel : public SensorModel
{
  public:
    ...
    KinectSensorModel (rw::sensor::CameraModel::Ptr camModel,
                       rw::sensor::Scanner25DModel::Ptr scanModel) :
        SensorModel (camModel->getName (), camModel->getFrame ()),
        _camModel (camModel), _scanModel (scanModel)
    {}
    static rw::core::Ptr< KinectSensorModel > make (rw::sensor::CameraModel::Ptr camModel,
                                                    rw::sensor::Scanner25DModel::Ptr scanModel)
    {
                                                     ...
                                                     int w, int h, double nearVal, double farVal)
{
    return rw::core::ownedPtr (new CameraModel (
        ProjectionMatrix::makePerspective (fov * Deg2Rad, w, h, nearVal, farVal), name, frame));
}
...
SimulatedKinect::SimulatedKinect (const std::string& name, rw::kinematics::Frame* frame) :
    ...
    _frameRate (1), _dtsum (0), _isAcquired (false), _isOpenned (false), _noiseEnabled (true),
    _near (0.01), _far (6), _fieldOfView (43), _grabSingleFrame (false), _width (640),
    _height (480), _img (new rw::sensor::Image (_width, _height, rw::sensor::Image::RGB,
                                                rw::sensor::Image::Depth8U)),
...
SimulatedKinect::SimulatedKinect (const std::string& name, const std::string& desc,
    ...
    SimulatedSensor (
        KinectSensorModel::make (makeDefaultCameraModel (name, frame, 43, 640, 480, 0.01, 6),
                                 makeDefaultScannerModel (name, frame, 43, 640, 480, 0.01, 6))),
    _frameRate (30), _dtsum (0), _isAcquired (false), _isOpenned (false), _noiseEnabled (true),
    _near (0.01), _far (6), _fieldOfView (43), _grabSingleFrame (false), _width (640),
    _height (480), _img (new rw::sensor::Image (_width, _height, rw::sensor::Image::RGB,
                                                rw::sensor::Image::Depth8U)),
    _scan (new rw::geometry::PointCloud (_width, _height))
...
SimulatedKinect::SimulatedKinect (rw::sensor::CameraModel::Ptr camModel,
...
bool SimulatedKinect::init (rw::graphics::SceneViewer::Ptr drawer)
{
    ...
    if (view->_camGroup->setOffscreenRenderEnabled (true)) {
        ...
        RW_WARN (
            "SimulatedKinect could not be initialized as offscreen rendering is not supported.");
        return false;
    }
    return true;
}
...
void SimulatedKinect::open ()
{
    if (_drawer == NULL)
        RW_THROW (
            "The SimulatedKinect sensor has not been properly initialized, please call init(...)");
    _isOpenned = true;
    _dtsum     = 0;
}
...
void SimulatedKinect::update (const Simulator::UpdateInfo& info, rw::kinematics::State& state)
{
    ...
    if (_frameRate < 0.00001) {
        if (_grabSingleFrame) {
            ...
            rw::math::Transform3D<> wTf =
                rw::kinematics::Kinematics::worldTframe (getFrame (), state);

            Image::Ptr img = _camModel->getImage (state);
            if (img == NULL) {
                img = ownedPtr (new rw::sensor::Image (
                    _width, _height, rw::sensor::Image::RGB, rw::sensor::Image::Depth8U));
...
SimulatedKinect::getSensorHandle (rwlibs::simulation::Simulator::Ptr simulator)
{
    return _rsensor;
}
