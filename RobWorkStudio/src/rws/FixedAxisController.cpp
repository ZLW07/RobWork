...
FixedAxisController::FixedAxisController (GLfloat NewWidth, GLfloat NewHeight) :
    _centerPt (0.0f, 0.0f), _stVec (0.0f, 0.0f, 0.0f), _enVec (0.0f, 0.0f, 0.0f)
{
    _viewTransform = inverse (
        Transform3D<>::makeLookAt (Vector3D<> (5, 5, 3), Vector3D<>::zero (), Vector3D<>::z ()));
...
rw::math::Quaternion< double > FixedAxisController::drag (float x, float y)
{
    ...
    if (perp.norm2 () > 1.0e-5) {    // if its non-zero
        ...
        math::Quaternion<> tmpQuat (perp (0), perp (1), perp (2), dot (_stVec, _enVec));
        return tmpQuat;
    }
    // if its zero
    ...
    if (e->type () == QEvent::MouseButtonPress) {
        ...
        if (event->buttons () == Qt::LeftButton) {
            if (event->modifiers () == Qt::ControlModifier) {
                ...
                // std::cout << event->y() << "-" << _lastPos(1) << "/" << _adjustHeight*10 << " "
                // << std::endl;
        ...
        if (event->buttons () == Qt::RightButton) {
            ...
            Vector3D<> translateVector ((event->x () - _lastPos (0)) / _width * 10,
                                        -((event->y () - _lastPos (1)) / _height * 10),
                                        0);
            _viewTransform.P () -= _viewTransform.R () * translateVector;
        }
        _lastPos (0) = event->x ();
