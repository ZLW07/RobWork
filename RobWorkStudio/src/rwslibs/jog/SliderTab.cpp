...
QSlider* makeHSlider (int end)
{
    ...
    const int pageStep     = (int) (pageScale * (slider->maximum () - slider->minimum ()));
...
*/
Slider::Slider (const std::string& title, double low, double high, QGridLayout* layout, int row,
                QWidget* parent) :
    QWidget (parent),
    _boxChanged (false), _sliderChanged (false), _toUnit (1.0)
{
    ...
    connect (_box, SIGNAL (valueChanged (double)), this, SLOT (boxValueChanged (double)));
...
Slider::~Slider ()
{
    // As the layout is not owned by this widget, we have to remove the following widgets
    // explicitly:
    delete _title;
    delete _lowLabel;
    delete _highLabel;
...
void Slider::setValue (double val)
{
    ...
    if (_low - 0.00001 <= val && val <= _high + 0.00001) {
        ...
        RW_WARN ("Jog joint value " << val << " out of range [" << _low << ", " << _high << "].");
    }
...
MovableFrameTab::MovableFrameTab (const std::pair< rw::math::Q, rw::math::Q >& bounds,
                                  MovableFrame* frame, rw::models::WorkCell* workcell,
                                  const rw::kinematics::State& state) :
    _state (state),
    _frame (frame), _updating (false)
{
...
void MovableFrameTab::setUnits (const std::vector< double >& converters,
...
{
    Transform3D<> parent2ref =
        Kinematics::frameTframe (_frame->getParent (_state), _refframe, _state);
...
JointSliderWidget::JointSliderWidget () : _enableAngularCombined (false)
{
    _layout = new QGridLayout (this);    // owned
}

void JointSliderWidget::setup (const std::vector< std::string >& titles,
                               const std::pair< Q, Q >& bounds, const Q& q, bool enablers,
                               bool enableAngularCombined)
{
    ...
    */
    ...
    for (size_t i = 0; i < bounds.first.size (); i++) {
        Slider* slider =
            new Slider (titles[i], bounds.first (i), bounds.second (i), _layout, (int) i + 2, this);
        slider->setValue (q (i));
        slider->unitUpdated ();
    ...
    do {
        ...
        try {    // Transform3D(Vector3D(0.2, 0.598, 0.85), Rotation3D(1, 0, 0, 0, 1, 0, -0, 0, 1))
            ...
            QMessageBox::critical (
                this, tr ("RobWorkStudio Jog"), tr ("Unable to parse '%1' as Q").arg (txt));
            continue;
        }
    } while (true);
}
...
void JointSliderWidget::setUnits (const std::vector< double >& converters,
                                  const std::vector< std::string >& descriptions)
{
...
TransformSliderWidget::TransformSliderWidget (const std::pair< rw::math::Q, rw::math::Q >& bounds,
                                              ...
                                              AngleType angleType, bool enablers) :
    _updating (false),
    _angleType (RPYtype), _carteasianbounds (bounds), _enablers (enablers), _lastChangedId (0)
{
    QGridLayout* tablayout = new QGridLayout (this);    // owned
...
void TransformSliderWidget::setUnits (const std::vector< double >& converters,
                                      const std::vector< std::string >& descriptions)
{
...
Q TransformSliderWidget::qFromTransform (const rw::math::Transform3D<>& transform)
{
    ...
    if (_angleType == QUAtype) {
        ...
        if (Quaternion< double > (qua + _QUAfromSliders).getLength () <
            Quaternion< double > (qua - _QUAfromSliders).getLength ()) {
            qua = -qua;
        }
        q[3] = qua.getQx ();
        q[4] = qua.getQy ();
...
rw::math::Transform3D<> TransformSliderWidget::getTransform ()
{
    ...
    switch (_angleType) {
        case QUAtype:
            return Transform3D<> (Vector3D<> (q[0], q[1], q[2]),
                                  Quaternion< double > (q[3], q[4], q[5], q[6]));
        case EAAtype:
            return Transform3D<> (Vector3D<> (q[0], q[1], q[2]), EAA<> (q[3], q[4], q[5]));
        case RPYtype: return q2transform (q);
        default: RW_THROW ("The type of angle has not been fully implemented.");
    }
}
...
{
    switch (i) {
        case TransformSliderWidget::RPYtype: return TransformSliderWidget::RPYtype;
        case TransformSliderWidget::EAAtype: return TransformSliderWidget::EAAtype;
        case TransformSliderWidget::QUAtype: return TransformSliderWidget::QUAtype;
        default: RW_THROW ("The type of angle is not supported!");
    }
}
    ...
    switch (_angleType) {
        case QUAtype:
            if (_lastTransformType == QUAtype) {
                ...
                transform          = Transform3D<> (Vector3D<> (q[0], q[1], q[2]),
                                           Quaternion< double > (q[3], q[4], q[5], q[6]));
            }
            break;
        case EAAtype:
...
TransformSliderWidget::QPair TransformSliderWidget::getBounds ()
{
    switch (_angleType) {
        ...
        case RPYtype: return _carteasianbounds;
        default: RW_THROW ("The type of angle has not been fully implemented.");
    }
}
...
CartesianDeviceTab::CartesianDeviceTab (const std::pair< rw::math::Q, rw::math::Q >& bounds,
                                        Device::Ptr device, WorkCell* workcell,
                                        const rw::kinematics::State& state) :
    _state (state),
    _device (device), _updating (true)
{
    QGridLayout* tablayout = new QGridLayout (this);    // owned
    QWidget* toppanel      = new QWidget ();
    ...
    const TransformSliderWidget::AngleType angleType =
        TransformSliderWidget::toAngleType (_cmbAngleType->currentIndex ());
    _transformSliderWidget = new TransformSliderWidget (
        bounds, Kinematics::frameTframe (_refFrame, _tcpFrame, _state), angleType, enablers);
    connect (_cmbAngleType,
             SIGNAL (currentIndexChanged (int)),
             _transformSliderWidget,
             SLOT (angleTypeChanged (int)));
...
void CartesianDeviceTab::setUnits (const std::vector< double >& converters,
                                   const std::vector< std::string >& descriptions)
{
    _transformSliderWidget->setUnits (converters, descriptions);
}
...
void CartesianDeviceTab::transformChanged (const Transform3D<>& refTtcp)
{
    ...
    if (const ParallelIKSolver::Ptr psolver = _iksolver.cast< ParallelIKSolver > ()) {
        ...
        targets[0] =
            ParallelIKSolver::Target (_tcpFrame, baseTtcp, _transformSliderWidget->enabledState ());
