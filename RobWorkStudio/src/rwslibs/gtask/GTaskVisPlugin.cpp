...
class RenderTargets : public Render
{
  public:
    ...
    void setTargets (std::vector< Target >& targets) { _targets = targets; }
...
void GTaskVisPlugin::initialize ()
{
    ...
    getRobWorkStudio ()->genericAnyEvent ().add (
        boost::bind (
            &GTaskVisPlugin::genericAnyEventListener, this, boost::arg< 1 > (), boost::arg< 2 > ()),
        this);
...
void GTaskVisPlugin::open (WorkCell* workcell)
{
    ...
    _targetDrawable = getRobWorkStudio ()->getWorkCellScene ()->addRender (
        "pointRender", _render, workcell->getWorldFrame ());
...
void GTaskVisPlugin::updateVis ()
{
    ...
    // for stats
    int droppedStat = 0, missedStat = 0, successStat = 0, slippedStat = 0, collisionStat = 0,
        collisionEnvStat = 0, collisionObjStat = 0, otherStat = 0;
    ...
    _targetDrawable =
        getRobWorkStudio ()->getWorkCellScene ()->addRender ("pointRender", _render, selframe);
    ...
    if (_showQuality->isChecked ()) {
        ...
        for (RenderTargets::Target& t : rtargets) {
            ...
            // std::cout << t.color[0] << " (" << t.scale << "+" << offset<<")*" << scale <<
            // std::endl;
        }
    }
    ...
    log ().info () << "Total:"
                   << successStat + slippedStat + otherStat + collisionStat + droppedStat +
                          missedStat
                   << " Succes:" << successStat << " Slipped:" << slippedStat
                   << " Dropped:" << droppedStat << " Missed:" << missedStat
                   << " Collision:" << collisionStat << " CollisionObj:" << collisionObjStat
                   << " CollisionEnv:" << collisionEnvStat << " Other:" << otherStat << "\n";
...
{
    const std::vector< RenderTargets::Target >& targets =
        ((RenderTargets*) _render.get ())->getTargets ();
    ...
    if (device != NULL) {
        ...
        if (targets[index].endTarget) {
            ...
            if (targets[index].ctarget.result != NULL &&
                targets[index].ctarget.result->gripperConfigurationGrasp.size () > 0) {
                q = targets[index].ctarget.result->gripperConfigurationGrasp;
            }
    ...
    if (targets[index].ctarget.result != NULL)
        log ().info () << "Quality of selected grasp: "
                       << targets[index].ctarget.result->qualityAfterLifting << std::endl;
...
void GTaskVisPlugin::genericAnyEventListener (const std::string& event, boost::any data)
{
    ...
    try {
        ...
        Log::warningLog () << "GTaskVisPlugin: Event \"" << event
