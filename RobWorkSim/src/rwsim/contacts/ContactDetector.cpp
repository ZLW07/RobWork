                ...
                for (Geometry::Ptr geoB : oB->getGeometry (state)) {
                    if (strategy.strategy->match (geoA->getGeometryData (),
                        ...
                        if (mapA.find (geoA->getId ()) == mapA.end ()) {
                            ...
                            std::pair< std::string, ContactModel::Ptr > pair =
                                std::pair< std::string, ContactModel::Ptr > (
                                    geoA->getId (), model.cast< ContactModel > ());
                            mapA.insert (pair);
                        }
                        if (mapB.find (geoB->getId ()) == mapB.end ()) {
                            ...
                            std::pair< std::string, ContactModel::Ptr > pair =
                                std::pair< std::string, ContactModel::Ptr > (
                                    geoB->getId (), model.cast< ContactModel > ());
                            mapB.insert (pair);
                        }
                    }
                }
...
ContactDetector::StrategyTable
ContactDetector::getContactStrategies (const std::string& frameA, const std::string& frameB) const
{
    StrategyTable res;
...
ContactDetector::StrategyTable ContactDetector::getContactStrategies (
    const std::string& frameA, rw::core::Ptr< const GeometryData > geometryA,
    const std::string& frameB, rw::core::Ptr< const GeometryData > geometryB) const
{
    StrategyTable res;
...
void ContactDetector::addContactStrategy (ProximitySetupRule rule, ContactStrategy::Ptr strategy,
                                          std::size_t pri)
{
...
void ContactDetector::addContactStrategy (ProximitySetup rules, ContactStrategy::Ptr strategy,
                                          std::size_t pri)
{
...
void ContactDetector::setDefaultStrategies (const PropertyMap& map)
{
    ...
    for (Object::Ptr object : objects) {
        for (Geometry::Ptr geom : object->getGeometry (state)) {
            ...
            if (gtype == GeometryData::SpherePrim)
                spheres++;
        }
    }
    std::size_t pri = 0;
        ...
        for (it = _strategies.begin (); (it != _strategies.end ()); it++) {
            ...
            if (match) {
                ...
                for (pairIt = geoPairs.begin (); pairIt < geoPairs.end (); pairIt++) {
                    ...
                    if (stratMatch.strategy->match (geoA->getGeometryData (),
                                                    geoB->getGeometryData ())) {
                        std::map< std::string, ContactModel::Ptr >& mapA =
                            stratMatch.models[*pair.first];
                        std::map< std::string, ContactModel::Ptr >& mapB =
                            stratMatch.models[*pair.second];
                        if (mapA.find (geoA->getId ()) == mapA.end ()) {
                        ...
                        ContactStrategyData& stratData =
                            data.getStrategyData (modelA.get (), modelB.get ());
                        contacts = stratMatch.strategy->findContacts (
                            modelA.get (), aT, modelB.get (), bT, stratData);
                ...
                for (pairIt = geoPairs.begin (); pairIt < geoPairs.end (); pairIt++) {
                    ...
                    if (stratMatch.strategy->match (geoA->getGeometryData (),
                        ...
                        if (log != NULL) {
                            ...
                            stratLog->setDescription (pair.first->getName () + "-" +
                                                      pair.second->getName ());
                        ...
                        ContactStrategyData& stratData =
                            data.getStrategyData (modelA.get (), modelB.get ());
                        ContactStrategyTracking& stratTracking =
                            tracking.getStrategyTracking (modelA.get (), modelB.get ());
                        contacts = stratMatch.strategy->findContacts (modelA.get (),
                                                                      aT,
                                                                      modelB.get (),
                                                                      bT,
                                                                      stratData,
                        ...
                        if (contacts.size () > 0) {
                            ...
                            info.frames = pair;
                            info.models = std::make_pair< ContactModel*, ContactModel* > (
                                modelA.get (), modelB.get ());
...
std::vector< Contact > ContactDetector::updateContacts (const State& state,
                                                        ContactDetectorData& data,
                                                        ContactDetectorTracking& tracking,
                                                        SimulatorLogScope* log)
{
    ...
    for (it = infos.begin (); it != infos.end (); it++) {
        ...
        const std::vector< Contact > contacts = info.strategy->updateContacts (
            modelA, aT, modelB, bT, stratData, *info.tracking, stratLog.get ());
...
void ContactDetector::constructTable (
    std::vector< std::vector< ContactDetector::Cell > >& table) const
{
...
void ContactDetector::printTable (const std::vector< std::vector< Cell > >& table,
                                  std::ostream& out, bool header)
{
    if (table.size () == 0)
        return;
    if (table[0].size () == 0)
