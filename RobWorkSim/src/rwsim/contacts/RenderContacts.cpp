...
struct RenderContacts::GLData
{
    ...
    ~GLData () { gluDeleteQuadric (quadratic); }
    GLUquadricObj* quadratic;
};

RenderContacts::RenderContacts () :
    _gl (new GLData ()), _showPoints (std::make_pair (true, true)),
...
RenderContacts::RenderContacts (const std::vector< Contact >& contacts) :
    _contacts (contacts), _gl (new GLData ()), _showPoints (std::make_pair (true, true)),
    _showNormals (std::make_pair (true, true))
{
    setSphereRadius ();
    setNormalLength ();
...
void RenderContacts::draw (const DrawableNode::RenderInfo& info, DrawableNode::DrawType type,
                           double alpha) const
{
