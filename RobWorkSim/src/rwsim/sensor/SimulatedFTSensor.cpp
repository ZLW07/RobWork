...
class FTSensorWrapper : public FTSensor
{
  ...
  public:
    FTSensorWrapper (SimulatedFTSensor* sensor, rwlibs::simulation::Simulator::Ptr sim) :
        FTSensor (sensor->getName ()), _sensor (sensor), _simHandle (sim)
    {}
...
void SimulatedFTSensor::addForceW (const Vector3D<>& point, const Vector3D<>& force,
                                   const Vector3D<>& cnormal, State& state, Body::Ptr body)
{
    const FTStateData& data = _sdata.get (state);
    addForce (data._bTw * point, data._bTw.R () * force, data._bTw.R () * cnormal, state, body);
}

void SimulatedFTSensor::addForce (const Vector3D<>& point, const Vector3D<>& force,
                                  const Vector3D<>& cnormal, State& state, Body::Ptr body)
{
...
void SimulatedFTSensor::addWrenchToCOM (const Vector3D<>& force, const Vector3D<>& torque,
                                        State& state, Body::Ptr body)
{
    FTStateData& data = _sdata.get (state);
    data._forceTmp += force;
...
void SimulatedFTSensor::addWrenchWToCOM (const Vector3D<>& force, const Vector3D<>& torque,
                                         State& state, Body::Ptr body)
{
    FTStateData& data = _sdata.get (state);
