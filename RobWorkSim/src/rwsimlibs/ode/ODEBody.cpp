...
ODEBody::ODEBody (dBodyID odeBody, RigidBody::Ptr rwbody, rw::math::Vector3D<> offset, int matID,
                  int conID) :
    _mframe (rwbody->getMovableFrame ()),
    _bodyId (odeBody), _body (rwbody), _rwframe (_mframe), _type (ODEBody::RIGID),
    _contactReductionThreshold (0.005),    // 1cm
    _materialID (matID), _contactID (conID), _geomId (0), _rwBody (rwbody), _kBody (NULL),
    _offset (offset)
{
    _body->changedEvent ().add (
        boost::bind (&ODEBody::bodyChangedListener, this, boost::arg< 1 > ()), this);
}

ODEBody::ODEBody (dBodyID odeBody, KinematicBody::Ptr kbody, int matID, int conID) :
    _mframe (kbody->getMovableFrame ()), _bodyId (odeBody), _body (kbody),
    _rwframe (kbody->getBodyFrame ()), _type (ODEBody::KINEMATIC),
    _contactReductionThreshold (0.005),    // 1cm
    _materialID (matID), _contactID (conID), _geomId (0), _rwBody (NULL), _kBody (kbody),
    _offset (0, 0, 0)
{
    _body->changedEvent ().add (
        boost::bind (&ODEBody::bodyChangedListener, this, boost::arg< 1 > ()), this);
...
ODEBody::ODEBody (std::vector< dGeomID > geomIds, dynamics::Body::Ptr body, int matID, int conID) :
    _mframe (NULL), _bodyId (0),    // a fixed object in ODE is allways part of the 0 body
    _body (body), _rwframe (body->getBodyFrame ()), _type (ODEBody::FIXED),
    _contactReductionThreshold (0.005),    // 1cm
    _materialID (matID), _contactID (conID), _geomId (geomIds[0]), _geomIds (geomIds),
    _rwBody (NULL), _kBody (NULL)
{
    _body->changedEvent ().add (
        boost::bind (&ODEBody::bodyChangedListener, this, boost::arg< 1 > ()), this);
}

ODEBody::ODEBody (dBodyID odeBody, dynamics::Body::Ptr body, rw::math::Vector3D<> offset, int matID,
                  int conID, ODEBodyType type) :
    _mframe (NULL),
    _bodyId (odeBody),    // a fixed object in ODE is allways part of the 0 body
    _body (body), _rwframe (body->getBodyFrame ()), _type (type),
    _contactReductionThreshold (0.005),    // 1cm
    _materialID (matID), _contactID (conID), _geomId (0), _rwBody (NULL), _kBody (NULL),
    _offset (offset)
{
...
ODEBody::ODEBody (dBodyID odeBody, rw::kinematics::Frame* frame) :
    _mframe (dynamic_cast< MovableFrame* > (frame)), _bodyId (odeBody), _body (NULL),
    _rwframe (frame), _type (ODEBody::RigidDummy), _contactReductionThreshold (0.005),    // 1cm
    _materialID (-1), _contactID (-1), _geomId (0), _rwBody (NULL), _kBody (NULL)
{}

void ODEBody::update (const rwlibs::simulation::Simulator::UpdateInfo& info,
                      rw::kinematics::State& state)
{
    ...
    switch (_type) {
        ...
        break;
        */
        default: RW_WARN ("UNSUPPORTED ODEBody type"); break;
    }
}
...
void ODEBody::postupdate (rw::kinematics::State& state)
{
    ...
    switch (_type) {
        case (ODEBody::RIGID): {
            ...
            Transform3D<> wTp =
                rw::kinematics::Kinematics::worldTframe (_mframe->getParent (state), state);
        ...
        case (ODEBody::FIXED): {
            // the fixed bodies are not allowed to move during simulation so we don't expect any
            // changes here
        } break;
        default: RW_WARN ("UNSUPPORTED ODEBody type"); break;
    }
...
ODEBody* ODEBody::makeRigidBody (dynamics::Body::Ptr rwbody, dSpaceID spaceId, ODESimulator* sim)
{
    ...
    std::vector< ODEUtil::TriGeomData* > gdatas =
        ODEUtil::buildTriGeom (geoms, spaceId, rwbody->getBodyFrame (), state, false);
    ...
    for (ODEUtil::TriGeomData* gdata : gdatas) {
        ...
        dGeomSetOffsetPosition (
            gdata->geomId, gdata->p[0] - mc[0], gdata->p[1] - mc[1], gdata->p[2] - mc[2]);
...
ODEBody* ODEBody::makeKinematicBody (Body::Ptr kbody, dSpaceID spaceid, ODESimulator* sim)
{
    ...
    std::vector< ODEUtil::TriGeomData* > gdatas =
        ODEUtil::buildTriGeom (geoms, spaceid, kbody->getBodyFrame (), state, false);
...
ODEBody* ODEBody::makeFixedBody (Body::Ptr rwbody, dSpaceID spaceid, ODESimulator* sim)
{
    ...
    std::vector< Geometry::Ptr > geoms = rwbody->getGeometry ();
    std::vector< ODEUtil::TriGeomData* > gdatas =
        ODEUtil::buildTriGeom (geoms, sim->getODESpace (), rwbody->getBodyFrame (), state, false);
    // if no triangles was loaded then continue
    if (gdatas.size () == 0) {
