...
void SimTaskVisPlugin::initialize ()
{
    ...
    getRobWorkStudio ()->genericAnyEvent ().add (
        boost::bind (&SimTaskVisPlugin::genericAnyEventListener,
                     this,
                     boost::arg< 1 > (),
                     boost::arg< 2 > ()),
        this);
...
void SimTaskVisPlugin::selectGrasp (int index)
{
    ...
    if (tcp != NULL && base != NULL) {
        ...
        std::cout << "bTf_e :" <<  inverse(Kinematics::worldTframe(base ,state)) *
        Kinematics::worldTframe(tcp ,state) << std::endl;
...
void SimTaskVisPlugin::genericAnyEventListener (const std::string& event, boost::any data)
{
    ...
    try {
        ...
        Log::warningLog () << "SimTaskVisPlugin: Event \"" << event
