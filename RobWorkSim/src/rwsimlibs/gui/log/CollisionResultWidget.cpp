...
CollisionResultWidget::CollisionResultWidget (rw::core::Ptr< const LogCollisionResult > entry,
                                              QWidget* parent) :
    SimulatorLogEntryWidget (parent),
    _ui (new Ui::CollisionResultWidget ()), _positions (entry->getPositions ()), _result (entry)
{
    _ui->setupUi (this);
    ...
    connect (_ui->_collisionPairs->selectionModel (),
             SIGNAL (selectionChanged (const QItemSelection&, const QItemSelection&)),
             this,
             SLOT (collisionPairsChanged (const QItemSelection&, const QItemSelection&)));
...
void CollisionResultWidget::updateEntryWidget ()
{
    ...
    for (const LogCollisionResult::ResultInfo& info : results) {
        ...
        _ui->_framePairTable->setItem (
            row, 2, new QTableWidgetItem (QString::number (info.result._collisionPairs.size ())));
        _ui->_framePairTable->setItem (
            row, 3, new QTableWidgetItem (QString::number (info.result._nrBVTests)));
        _ui->_framePairTable->setItem (
            row, 4, new QTableWidgetItem (QString::number (info.result._nrPrimTests)));
        row++;
    }
    _ui->_framePairTable->setSortingEnabled (true);
    if (nrOfEntries > 0)
        _ui->_framePairTable->setRangeSelected (
            QTableWidgetSelectionRange (0, 0, nrOfEntries - 1, 4), true);
}
...
void CollisionResultWidget::framePairsChanged (const QItemSelection& newSelection,
                                               const QItemSelection& oldSelection)
{
    ...
    foreach (const QModelIndex& index, indexes)
    {
        if (index.column () > 0)
            continue;
        const LogCollisionResult::ResultInfo& info =
        ...
        for (std::size_t i = 0; i < info.result._collisionPairs.size (); i++) {
            ...
            _ui->_collisionPairs->setItem (
                row, 2, new QTableWidgetItem (QString::number (pair.size)));
            row++;
        }
    }
    _ui->_collisionPairs->setSortingEnabled (true);
    if (_ui->_collisionPairs->rowCount () > 0)
        _ui->_collisionPairs->setRangeSelected (
            QTableWidgetSelectionRange (0,
                                        0,
                                        _ui->_collisionPairs->rowCount () - 1,
                                        _ui->_collisionPairs->columnCount () - 1),
            true);
}

void CollisionResultWidget::collisionPairsChanged (const QItemSelection& newSelection,
                                                   const QItemSelection& oldSelection)
{
    ...
    foreach (const QModelIndex& index, indexes)
    {
        ...
        const CollisionStrategy::Result::CollisionPair& collPair =
            result.result._collisionPairs[collPairId];
    ...
    const DrawableNode::Ptr drawable =
        _graph->makeDrawable ("Collisions", render, DrawableNode::Virtual);
...
CollisionResultWidget::Dispatcher::makeWidget (rw::core::Ptr< const SimulatorLog > entry,
                                               QWidget* parent) const
{
    const rw::core::Ptr< const LogCollisionResult > tentry =
        entry.cast< const LogCollisionResult > ();
    if (!(tentry == NULL))
