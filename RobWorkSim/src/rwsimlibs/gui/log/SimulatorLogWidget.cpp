                                           ...
                                           const QItemSelection& deselected)
{
    ...
    foreach (const QModelIndex& index, selected.indexes ())
    {
        ...
        /*
                        const PhysicsEngineInternalInfo::LeafEntry* const leaf = dynamic_cast<const
           PhysicsEngineInternalInfo::LeafEntry*>(entry); const
           PhysicsEngineInternalInfo::CompositeEntry* const composite = dynamic_cast<const
           PhysicsEngineInternalInfo::CompositeEntry*>(entry); if (leaf != NULL) {
                                _ui->_lines->setText(QString::number(leaf->line));
                                const PhysicsEngineInternalInfo::ContactSet* const typed =
           dynamic_cast<const PhysicsEngineInternalInfo::ContactSet*>(leaf); if (typed != NULL) {
                                        ContactSetWidget* const widget = new
           ContactSetWidget(_dwc,_ui->_pageStack); widget->setContactSet(typed);
                                        widget->showGraphics(_root,_glview->getScene());
                                ...
                                const PhysicsEngineInternalInfo::Positions* const pos =
           dynamic_cast<const PhysicsEngineInternalInfo::Positions*>(leaf); const
           PhysicsEngineInternalInfo::Velocities* const vel = dynamic_cast<const
           PhysicsEngineInternalInfo::Velocities*>(leaf); if (pos != NULL) { BodyMotionWidget* const
           widget = new BodyMotionWidget(_dwc,_ui->_pageStack); widget->setPositions(pos);
                                        widget->showGraphics(_root,_glview->getScene());
                                ...
                                if (vel != NULL) {
                                        BodyMotionWidget* const widget = new
           BodyMotionWidget(_dwc,_ui->_pageStack); widget->setVelocities(vel);
                                        widget->showGraphics(_root,_glview->getScene());
                                        widget->updateBodyMotionWidget();
                                ...
                                _ui->_lines->setText(QString::number(composite->line.first) + " to "
